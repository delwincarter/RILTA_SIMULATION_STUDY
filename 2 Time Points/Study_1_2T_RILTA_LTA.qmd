---
title: "Study 1 (k = 2) RILTA Generated, LTA Analyzed"
format:
  html:
    code-fold: true
editor: visual
...
csv_directory <- here('2 Time Points', '2_2T_RILTA_GEN_LTA_ANALYZED')
...
output_folder <- "4. 2T RILTA GEN LTA ANALYZED"  # Replace with your folder path
...
### Step8: Scrape Rows and Process Data
...
create_plot <- function(data, title_suffix) {
  ...
  colors <- c(
    "Parameter Bias" = "#7030A0", 
    "Standard Error Bias" = "#C830CC", 
    "Coverage Failure" = "#7030A0", 
    "Power Failure" = "black"
  )
  shapes <- c(
    "Parameter Bias" = 16,  # Solid circle
    "Standard Error Bias" = 18,  # Diamond
    "Coverage Failure" = 1,  # Hollow circle
    "Power Failure" = 4  # X shape
  )
  ...
  # Set Y-axis limits and breaks based on title_suffix
  if (title_suffix == "Stayer") {
    y_limits <- c(-40, 40)
    y_breaks <- seq(-40, 40, by = 10)
  } else {
    y_limits <- NULL  # No specific limits for "Mover"
    y_breaks <- seq(-40, 160, by = 40)
  }

  base_plot <- ggplot(data = data, aes(x = Lambda, group = Population_Label)) +  
    geom_line(aes(y = Parameter_Bias, color = "Parameter Bias"), linewidth = 0.7, linetype = "solid") +  
    geom_line(aes(y = SE_Bias, color = "Standard Error Bias"), linewidth = 0.7, linetype = "solid") +  
    geom_point(aes(y = Parameter_Bias, color = "Parameter Bias"), shape = 16, size = 1.2, alpha = 1) +  
    geom_point(aes(y = SE_Bias, color = "Standard Error Bias"), shape = 18, size = 1.4, alpha = 1) +  # Red diamonds
    geom_point(data = subset(data, Coverage_Dic == 0), aes(y = Parameter_Bias, color = "Coverage Failure"), shape = 1, size = 2, alpha = 1) +  
    geom_point(data = subset(data, Power_Dic == 0), aes(y = Parameter_Bias, color = "Power Failure"), shape = 4, size = 2, alpha = 1) + 
    scale_color_manual(
...
ggsave(here("2 Time Points", "zFIGURES", "x2t_rilta_lta_plots", "plot_mover.svg"), plot = plot_mover_no_title, width = 6, height = 4, dpi = 300, device = "svg")
```
...
ggsave(here("2 Time Points", "zFIGURES", "x2t_rilta_lta_plots", "plot_stayer.svg"), plot = plot_stayer_no_title, width = 6, height = 4, dpi = 300, device = "svg")
```
...
```{r create-heatmap-table, message=FALSE}
