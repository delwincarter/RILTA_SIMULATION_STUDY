...
output_folder <- "4_2T_RILTA_GEN_RILTA_ANALYZED"
...
### **Step 10 : Summarize Replications with Errors**

#### **Objective**

*Calculate the percentage of Errors for each file, handling missing values appropriately.*

#### **Step 10 Part 1: Data Processing**
...
#### Step 10 part 2: Initial Visualization
...
#### Step 10 Part 4: Create DF with Reps with errors Flagged
...
#### Step 10 Part 5: Remove Error Rows and Calculate Remaining Replications
...
#### Step 10 Part 6: Ensure All Files Without Errors Have Replications Listed

```{r, eval=FALSE}

#| label: "ensure-no-error-files"
#| echo: true
#| message: false
#| warning: false
#| code-fold: true

# Merge RemainingReplications into error_summary
error_summary <- error_summary %>%
  left_join(remaining_replications, by = "FileName") %>%
  mutate(
    ComputedAfterErrors = CompletedReplications - ErrorReplications
...
if (length(files_without_errors) > 0) {
  all_reps_without_errors <- all_reps %>%
    filter(FileName %in% files_without_errors) %>%
    mutate(ErrorFlag = 0,  # Explicitly set ErrorFlag to 0 for these rows
           Rep = as.character(Rep))  # Convert Rep to character for consistency

  clean_combined_data <- clean_combined_data %>%
    mutate(Rep = as.character(Rep))  # Ensure Rep is character in the existing dataset

  clean_combined_data <- bind_rows(clean_combined_data, all_reps_without_errors)
}
...
#### Step 10 Part 7: Generate Error Handling Summary Table
...
# Step 7: Generate the Error Handling Summary Table
error_handling_summary_table <- error_summary %>%
  gt() %>%
  tab_header(
    title = "Error Handling Summary",
  ...
  fmt_number(
    columns = c(RequestedReplications, CompletedReplications, RemainingReplications,
                ErrorReplications, ComputedAfterErrors, ErrorRate),
    decimals = 2
  ) %>%
  tab_options(
    table.font.size = "small",
...
### **Step 11: Summarize and Delete cases with Label Switching**

#### **Objective**

*Summarize cases with label switching and delete cases*

#### Part 1 Create Column Names from the Filename
...
violation_summary <- final_data_with_actuals %>%
  mutate(
    Any_Violation = ifelse(is.na(Any_Violation), 0, Any_Violation) # Replace NA with 0 for violations
  ) %>%
  group_by(FileName, Population, N, Lambda) %>%
  summarize(
    Total_Rows = n(),                                # Total runs
...
# Add TPs (logit values) and clean `N_numeric`
filtered_violation_summary <- violation_summary %>%
  mutate(
    TPs = case_when(
      `Transition Probability` == ".800" ~ 1.385,
...
# Summarize and visualize the final table
violation_table <- filtered_violation_summary %>%
  select(
    `Transition Probability`,               # Transition probabilities
    TPs,                                    # Logit values
    Lambda,                                 # Lambda values
    ...
    `Total Mplus Runs` = Total_Rows,        # Total runs
    Total_Violations,                       # Total violations
    `% of Violations` = Percentage_Violations
  ) %>%
  gt() %>%
  tab_header(
    title = "Monte Carlo Results:",
...
#### Step 11 Part 4: Delete Cases that Violate\*\*
