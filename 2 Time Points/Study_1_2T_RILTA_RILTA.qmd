...
plot_violator <- function(i) {
  ...
  ggsave(filename = file.path("2 Time Points", "zVIOLATOR_PLOTS", "z2t_rilta_rilta_violator_plots", paste0("violator_plot_", i, "_", file_name, ".png")),
...
extract_all_reps <- function(filepath) {
  lines <- readLines(filepath)
...
calculate_replication_summary <- function(all_reps) {
  ...
  full_summary <- requested_replications %>%
    left_join(all_reps, by = "FileName") %>%
...
clusterExport(cl, c("extract_errors_from_file", "extract_all_reps", "extract_requested_replications"))
clusterEvalQ(cl, library(tidyverse))

error_summary <- bind_rows(parLapply(cl, file_list, extract_errors_from_file))
all_reps <- bind_rows(parLapply(cl, file_list, extract_all_reps))
requested_replications <- bind_rows(parLapply(cl, file_list, extract_requested_replications))

stopCluster(cl)

replication_summary <- calculate_replication_summary(all_reps, all_reps)
...
The purposes of this chunk are as follows:

-   Scrape output files and retrieve specific data

    -   Number or errors per replication

    -   Number of requested replications

    -   Number of completed replications

-   Prepare a data frame (all_reps) that can be used downstream for visualization, deletion of cases with errors, and merging with a previous dataframe that can then be used to calculate and visualize Monte Carlo simulation estimates
...
#### Step 10 Part 5: Visualize the data post errors
