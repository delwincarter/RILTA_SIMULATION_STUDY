...
*Load all CSV files and combine them into a single data frame using a child document for all .qmd files.*
...
### Step 3: Convert Logits to Probabilities and Add Actual (Population) Values and flag violations
...
output_folder <- "3. 2T RILTA GEN RILTA ANALYZED"  # Replace with your folder path
...
# CHECK FOR LABEL SWITCHING PART 2

### Step 7: Combine All CSV Files into One Data Frame From the new data

**Objective:** 

*Load all new CSV files and combine them into a single data frame using a child document for all .qmd files.*
...
### Step 9: Convert Logits to Probabilities and Add Actual (Population) Values and flag violations
...
# Step 4: Parallelized Processing
output_folder <- "3. 2T RILTA GEN RILTA ANALYZED_REP"  # Replace with your folder path
file_list <- list.files(output_folder, pattern = "\\.out$", full.names = TRUE)
...
SAVE THIS FOR LATER for error deletion
...
final_results <- normalize_filenames(final_results)
final_data_with_actuals <- normalize_filenames(final_data_with_actuals)

# Convert Rep columns to the same type for consistent comparison
final_data_with_actuals$Rep <- as.character(final_data_with_actuals$Rep)
final_results$Rep <- as.character(final_results$Rep)

# Step 2: Remove rows from final_data_with_actuals that have errors
clean_combined_data <- final_data_with_actuals %>%
  anti_join(final_results %>% filter(ErrorFlag == 1), by = c("FileName", "Rep"))

# Step 3: Calculate remaining replications for each condition
remaining_replications <- clean_combined_data %>%
  group_by(FileName) %>%
  summarise(RemainingReplications = n(), .groups = "drop")

# Step 4: Calculate error summary for each file
error_summary <- final_results %>%
  group_by(FileName) %>%
  summarise(
    Requested = first(Requested),
    Completed = first(Completed),
    ErrorReplications = sum(ErrorFlag == 1, na.rm = TRUE),
    .groups = "drop"
  ) %>%
  left_join(remaining_replications, by = "FileName") %>%
  mutate(
    RemainingReplications = coalesce(RemainingReplications, 0),
    ComputedAfterErrors = Completed - ErrorReplications,
    ErrorRate = if_else(Completed > 0, (ErrorReplications / Completed) * 100, 0)
  )

# Step 5: Create the GT table for the combined data
error_handling_summary_table <- error_summary %>%
  ...
  fmt_number(
    columns = c("Requested", "Completed", "ErrorReplications", "RemainingReplications", "ComputedAfterErrors", "ErrorRate"),
    decimals = 2
  )

# Step 6: Display the GT table
print(error_handling_summary_table)

```

## STEP 11: DELETE CASES WITH ERRORS
...
#### Step 11: Part 1 Create Column Names from the Filename
...
#### Step 11: Part 2 Calculate Violation Percentages per Condition
...
#### Step 11: Part 3 Summarize & Visualize Label Switching Percentage Results
...
#### \*\* Step 12: Delete Cases that Violate\*\*
...
### **Step 13: Compute Monte Carlo (MC) Values**
...
### **Step 14: Calculate Dichotomous Variables and Bias**
...
### **Step 15: Subset Data for Movers and Stayers**
...
### **Step 16: Combined Plot for Mover and Stayer**
...
### **Step 17: Prepare Data for Heatmaps**
...
### **Step 18: Heatmap Creation and Rendering**
