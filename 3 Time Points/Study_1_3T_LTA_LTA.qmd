---
title: "Study 1 (k = 2) LTA Generated, LTA Analyzed; Three Timepoints"
format:
  html:
    code-fold: true
editor: visual
...
lta_lta_func <- function(N, TPs, TH) {
  ...
  LTA_LTA_Model<- mplusModeler(LTA_LTA, 
                                   dataout = here("5. 3T LTA GEN LTA ANALYZED", glue("LTA_LTA_N_{N}_TP_{TPs}_TH_{TH}_WD.dat")),
                                   modelout = glue(here("5. 3T LTA GEN LTA ANALYZED", "LTA_LTA_N_{N}_TP_{TPs}_TH_{TH}_WD.inp")),
...
csv_directory <- here('3 Time Points', '5_3T_LTA_GEN_LTA_ANALYZED')
...
plot_violator <- function(i) {
  ...
  ggsave(filename = file.path("z3t_lta_lta_violator_plots", paste0("violator_plot_", i, "_", file_name, ".png")),
...
output_folder <- "5. 3T LTA GEN LTA ANALYZED"  # Adjust to your folder path
...
lta_lta_func <- function(N_numeric, TPs, Replications_Needed) {
  ...
  # Run Mplus model
  LTA_LTA_Model<- mplusModeler(LTA_LTA, 
                                   dataout = here("5. 3T LTA GEN LTA ANALYZED_REP", glue("LTA_LTA_N_{N_numeric}_TP_{TPs}_TH_1.dat")),
                                   modelout = glue(here("5. 3T LTA GEN LTA ANALYZED_REP", "LTA_LTA_N_{N_numeric}_TP_{TPs}_TH_1.inp")),
                                   check = TRUE, run = TRUE, hashfilename = FALSE)
return(LTA_LTA_Model)
}
...
# Step 1: Set the correct CSV directory
csv_directory <- here('3 Time Points', '5_3T_LTA_GEN_LTA_ANALYZED_REP')
...
### **Step 9: Summarize Errors and Violations**
...
gt_table |> tab_options(table.width = pct(75)) |> gtsave(here("z2t_heatmaps", "2T_L_L.png"))
```

```{r}
# Extract models
lta_sim <- readModels(here("5. 3T LTA GEN LTA ANALYZED"), what = "parameters")
...
create_plot <- function(data, title_suffix) {
  ggplot(data = data, aes(x = factor(N))) +  
    ...
    geom_point(aes(y = Parameter_Bias, color = "Parameter Bias"), shape = 16, size = 1.5, fill = "#4169E1", alpha = 0.8) +  
    geom_point(aes(y = SE_Bias, color = "Standard Error Bias"), shape = 18, size = 1.5, fill = "#E14169", alpha = 0.8) +  
    geom_point(data = subset(data, Coverage_Dic == 0), aes(y = Parameter_Bias, color = "Coverage Failure"), shape = NA, size = 1.5, fill = "#4169E1", alpha = 0.8) +  
    geom_point(data = subset(data, Power_Dic == 0), aes(y = Parameter_Bias, color = "Power Failure"), shape = NA, size = 1.5, fill = "black", alpha = 0.8) + 
    geom_line(data = subset(data, Coverage_Dic == 0), aes(y = Parameter_Bias, color = "Coverage Failure", group = Population_Label), linewidth = 0.3, linetype = "solid") +
    geom_line(data = subset(data, Power_Dic == 0), aes(y = Parameter_Bias, color = "Power Failure", group = Population_Label), linewidth = 0.3, linetype = "solid") +
    scale_color_manual(
    ...
    labs(
      ...
      y = "Bias (%)",
      color = "",
      title = paste("3T LTA Generated, LTA Analyzed with", title_suffix, "Transition Probabilities")
    ) +
    coord_cartesian(ylim = c(-40, 40)) +  
...
ggsave(here("3 Time Points", "zFIGURES", "x3t_lta_lta_plots", "plot_mover.svg"), plot = plot_mover_no_title, width = 6, height = 3, dpi = 300, device = "svg")
```
...
ggsave(here("3 Time Points", "zFIGURES", "x3t_lta_lta_plots", "plot_stayer.svg"), plot = plot_stayer_no_title, width = 6, height = 3, dpi = 300, device = "svg")
```
...
create_gt_table <- function(subset, transition_probability) {
  ...
  gt_table <- subset %>%
    ...
    opt_table_font(stack = "geometric-humanist") %>% 
    tab_header(
      title = paste("3T LTA Generated & LTA Analyzed with Transition Probability of", transition_probability)
    ) %>%
    cols_label(
      N_Label = "Sample Size",
      average = "Estimated<br>Probability",
...
# Further operations on gt_table
gt_table |> tab_options(table.width = pct(65)) |> gtsave(here("3 Time Points", "zHEATMAPS", "z3t_heatmaps", "z3t_l_l_heatmaps", "3T_L_L.png", expand = 8))
