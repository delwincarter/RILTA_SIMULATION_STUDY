...
lta_rilta_func <- function(N, TPs, TH) {
  ...
  LTA_RILTA_Model<- mplusModeler(LTA_RILTA, 
                                   dataout = here("6. 3T LTA GEN RILTA ANALYZED", glue("LTA_RILTA_N_{N}_TP_{TPs}_TH_{TH}_WD.dat")),
                                   modelout = glue(here("6. 3T LTA GEN RILTA ANALYZED", "LTA_RILTA_N_{N}_TP_{TPs}_TH_{TH}_WD.inp")),
...
csv_directory <- here('3 Time Points', '6_3T_LTA_GEN_RILTA_ANALYZED')
...
plot_violator <- function(i) {
  ...
  ggsave(filename = file.path("z3t_lta_rilta_violator_plots", paste0("violator_plot_", i, "_", file_name, ".png")),
...
extract_errors_and_replications <- function(filepath) {
  ...
  if (length(lines) == 0 || !any(grepl("Number of replications", lines))) {
    message("File flagged as empty or malformed: ", basename(filepath))
    return(tibble(
      FileName = basename(filepath),
      Replication = NA,
      ErrorFlag = 1  # Mark as error since the file is malformed
    ))
  }
  
  # Extract replication numbers
  replication_info_line <- grep("Number of replications", lines)
  requested_replications <- as.integer(str_extract(lines[replication_info_line + 1], "\\d+"))
  completed_replications <- as.integer(str_extract(lines[replication_info_line + 2], "\\d+"))
  
  results <- vector("list", completed_replications)  # Pre-allocate list to store tibbles for each replication

  # Initialize results list with no error flag
  for (i in seq_len(completed_replications)) {
    results[[i]] <- tibble(
      FileName = basename(filepath),
      Replication = i,
      ErrorFlag = 0
  ...
  # Check for errors in each replication
  error_keywords <- c("SINGULARITY", "NON-POSITIVE DEFINITE")
  current_replication <- NA
  for (line in lines) {
    # Detect replication start
    if (grepl("REPLICATION", line)) {
      current_replication <- as.integer(str_extract(line, "\\d+"))
    }
    
    # Check for errors and update the flag
    if (!is.na(current_replication) && current_replication <= completed_replications &&
        any(sapply(error_keywords, grepl, x=line))) {
      results[[current_replication]]$ErrorFlag <- 1
    }
  }
...
output_folder <- "7. 3T RILTA GEN RILTA ANALYZED_REP"
...
# Step 3: Combine results and create a summary table
final_results <- bind_rows(all_errors)

# Rename Replication to Rep in final_results to match final_data_with_actuals
final_results <- final_results %>%
  rename(Rep = Replication)
...
error_summary <- final_results %>%
  ...
  summarise(
    ...
    Errors = sum(ErrorFlag == 1),  # Sum up errors per file
    GoodReplications = Completed - Errors,
    ErrorRate = (Errors / Completed) * 100
  )

# Step 5: Create and display the GT table
error_summary_table <- error_summary %>%
  gt() %>%
  tab_header(
    title = "Error and Replication Summary",
    subtitle = "Comprehensive analysis for each output file"
  ) %>%
  cols_label(
  ...
  fmt_number(columns = vars(Requested, Completed, Errors, GoodReplications, ErrorRate), decimals = 2)

# Step 6: Display the Table or Notify
if (any(error_summary$Errors > 0)) {
  print(error_summary_table)  # This is where your print function is now properly placed
} else {
...
lta_rilta_func <- function(N_numeric, TPs, Replications_Needed) {
  ...
  # Run Mplus model
  LTA_RILTA_Model<- mplusModeler(LTA_RILTA, 
                                   dataout = here("6. 3T LTA GEN RILTA ANALYZED_REP", glue("LTA_RILTA_N_{N_numeric}_TP_{TPs}_TH_1_WD.dat")),
                                   modelout = glue(here("6. 3T LTA GEN RILTA ANALYZED_REP", "LTA_RILTA_N_{N_numeric}_TP_{TPs}_TH_1_WD.inp")),
                                   check = TRUE, run = TRUE, hashfilename = FALSE)
return(LTA_RILTA_Model)
}
...
# Step 1: Set the correct CSV directory
csv_directory <- here('3 Time Points', '6_3T_LTA_GEN_RILTA_ANALYZED_REP')
...
ggsave(here("3 Time Points", "zFIGURES", "x3t_lta_rilta_plots", "plot_mover.svg"), plot = plot_mover_no_title, width = 6, height = 3, dpi = 300, device = "svg")
```
...
ggsave(here("3 Time Points", "zFIGURES", "x3t_lta_rilta_plots", "plot_stayer.svg"), plot = plot_stayer_no_title, width = 6, height = 3, dpi = 300, device = "svg")
```
...
gt_table |> 
  ...
  gtsave(here("3 Time Points", "zHEATMAPS", "z3t_l_r_heatmaps", "3T_L_R.png"))
