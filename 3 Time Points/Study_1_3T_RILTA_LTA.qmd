---
title: "Study 1 (k = 2) RILTA Generated, LTA Analyzed; Three Timepoints"
format:
  html:
    code-fold: true
editor: visual
...
# Study 1 (k = 2): RILTA Generated, LTA Analyzed
...
rilta_lta_func <- function(N, TPs, TH, Lambda) {
  ...
  RILTA_LTA <- mplusObject(
    TITLE = glue("Generate RILTA_LTA_N_{N}_TP_{TPs}_TH_{TH}"),
  ...
  RILTA_LTA_Model<- mplusModeler(RILTA_LTA, 
                                   dataout = here("8. 3T RILTA GEN LTA ANALYZED", glue("RILTA_LTA_N_{N}_TP_{TPs}_TH_{TH}_Lambda_{Lambda}.dat")),
                                   modelout = glue(here("8. 3T RILTA GEN LTA ANALYZED", "RILTA_LTA_N_{N}_TP_{TPs}_TH_{TH}_Lambda_{Lambda}.inp")),
...
csv_directory <- here('3 Time Points', '7_3T_RILTA_GEN_LTA_ANALYZED')
...
plot_violator <- function(i) {
  ...
  ggsave(filename = file.path("z3t_rilta_lta_violator_plots", paste0("violator_plot_", i, "_", file_name, ".png")),
...
extract_errors_from_file <- function(filepath) {
  ...
  # Return a default result if no errors/warnings are found
  if (length(results) == 0) {
    return(tibble(
      FileName = basename(filepath),
      Replication = NA,
...
extract_completed_replications <- function(filepath) {
  lines <- readLines(filepath)
  completed_line <- lines[grepl("Completed", lines)]
...
# Step 3: Calculate Replication Summary with Error Rate
calculate_replication_summary <- function(error_summary, completed_replications, requested_reps = 500) {
  summary <- error_summary %>%
    ...
    summarise(
      ...
      WarningReplications = n_distinct(Replication[MessageType == "Warning"]),
      .groups = "drop"
    )
  ...
  full_summary <- completed_replications %>%
    ...
    mutate(
      ...
      WarningReplications = coalesce(WarningReplications, 0),
      GoodReplications = CompletedReplications - ErrorReplications - WarningReplications,
      ErrorRate = if_else(TotalReplications > 0, (ErrorReplications / TotalReplications) * 100, 0)
    ) %>%
    select(FileName, CompletedReplications, TotalReplications, ErrorReplications, WarningReplications, GoodReplications, ErrorRate)
  
  # Filter for files with errors
  filtered_summary <- full_summary %>%
    filter(ErrorReplications > 0 | WarningReplications > 0)

  return(filtered_summary)
}

# Step 4: Parallelized Processing
output_folder <- "8. 3T RILTA GEN LTA ANALYZED"  # Adjust to your folder path
...
replication_summary <- calculate_replication_summary(error_summary, completed_replications, requested_reps = 500)

# Step 8: Display the Table or Notify
if (nrow(replication_summary) > 0) {
  replication_summary_table <- replication_summary %>%
    ...
    fmt_number(columns = c(CompletedReplications, TotalReplications, ErrorReplications, WarningReplications, GoodReplications, ErrorRate), decimals = 2) %>%
    cols_label(
      FileName = "File Name",
      ...
      WarningReplications = "Replications with Warnings",
...
Lambda = case_when(
  ...
  grepl("lambda_1(\\.|$)", FileName) ~ "1",      # Match 'Lambda_1' followed by a dot or end of string
  grepl("lambda_0(\\.|$)", FileName) ~ "0",      # Match 'Lambda_0' followed by a dot or end of string
  TRUE ~ NA_character_
),
...
rilta_lta_func <- function(N_numeric, TPs, Lambda, Replications_Needed) {
  ...
  # Run Mplus model
  RILTA_LTA_Model<- mplusModeler(RILTA_LTA, 
                                   dataout = here("8. 3T RILTA GEN LTA ANALYZED_REP", glue("RILTA_LTA_N_{N_numeric}_TP_{TPs}_TH_1_Lambda_{Lambda}.dat")),
                                   modelout = glue(here("8. 3T RILTA GEN LTA ANALYZED_REP", "RILTA_LTA_N_{N_numeric}_TP_{TPs}_TH_1_Lambda_{Lambda}.inp")),
                                   check = TRUE, run = TRUE, hashfilename = FALSE)
return(RILTA_LTA_Model)
}
...
# Step 1: Set the correct CSV directory
csv_directory <- here('3 Time Points', '7_3T_RILTA_GEN_LTA_ANALYZED_REP')
...
#### Step 9: Part 1 Create Column Names from the Filename
...
#### Step 9: Part 2 Calculate Violation Percentages per Condition
...
create_plot <- function(data, title_suffix) {
  ...
  base_plot <- ggplot(data = data, aes(x = Lambda, y = Parameter_Bias, color = "Parameter Bias", group = Population_Label)) +  
    ...
    geom_point(aes(y = SE_Bias), shape = 18, size = 1, fill = "#E14169", alpha = 0.8) +  
    ...
    scale_y_continuous(breaks = seq(-40, 160, by = 40)) +  # Default Y-axis scale for other plots
    theme_minimal() +
    theme(
      panel.grid.major.x = element_blank(),
      panel.grid.minor.x = element_blank(),
  ...
  # Apply specific Y-axis limits and breaks for "Stayer" plot
  if (title_suffix == "Stayer") {
    base_plot <- base_plot +
      coord_cartesian(ylim = c(-40, 40)) +  # Set Y-axis limits
      scale_y_continuous(breaks = seq(-40, 40, by = 10))  # Set Y-axis breaks for "Stayer"
  }
...
ggsave(here("3 Time Points", "zFIGURES", "x3t_rilta_lta_plots", "plot_mover.svg"), plot = plot_mover_no_title, width = 6, height = 3, dpi = 300, device = "svg")
```
...
ggsave(here("3 Time Points", "zFIGURES", "x3t_rilta_lta_plots", "plot_stayer.svg"), plot = plot_stayer_no_title, width = 6, height = 3, dpi = 300, device = "svg")
```
...
subset_02_table
subset_02_table |>  tab_options(table.width = pct(65)) |> gtsave(here("3 Time Points", "zHEATMAPS", "z3t_heatmaps","z3t_r_l_heatmaps", "2T_R_L_.200.png"))
```
...
subset_03_table
subset_03_table |>  tab_options(table.width = pct(65)) |> gtsave(here("3 Time Points", "zHEATMAPS", "z3t_heatmaps","z3t_r_l_heatmaps","2T_R_L_.300.png"))
```
...
subset_04_table
subset_04_table |>  tab_options(table.width = pct(65)) |> gtsave(here("3 Time Points", "zHEATMAPS", "z3t_heatmaps","z3t_r_l_heatmaps","2T_R_L_.400.png"))
```
...
subset_06_table
subset_06_table |>  tab_options(table.width = pct(65)) |> gtsave(here("3 Time Points", "zHEATMAPS", "z3t_heatmaps","z3t_r_l_heatmaps","2T_R_L_.600.png"))
```
...
subset_07_table
subset_07_table |>  tab_options(table.width = pct(65)) |> gtsave(here("3 Time Points", "zHEATMAPS","z3t_r_l_heatmaps","2T_R_L_.700.png"))
```
...
subset_08_table
subset_08_table |>  tab_options(table.width = pct(65)) |> gtsave(here("3 Time Points", "zHEATMAPS", "z3t_r_l_heatmaps","2T_R_L_.800.png"))
```
