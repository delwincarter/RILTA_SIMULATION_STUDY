...
csv_directory <- here("8. 3T RILTA GEN LTA ANALYZED")
...
process_chunk <- function(chunk_start, data) {
  if (chunk_start + 8 <= nrow(data)) {
    ...
    se11 <- row6[6]                              # Column 6 from row 5 (SE_11)
...
stopCluster(cl)  # Stop the cluster after processin
```
...
subset_02_table
subset_02_table |>  tab_options(table.width = pct(65)) |> gtsave(here("z3t_r_l_heatmaps", "2T_R_L_.200.png"))
...
subset_03_table
subset_03_table |>  tab_options(table.width = pct(65)) |> gtsave(here("z3t_r_l_heatmaps","2T_R_L_.300.png"))
```
...
subset_04_table
subset_04_table |>  tab_options(table.width = pct(65)) |> gtsave(here("z3t_r_l_heatmaps","2T_R_L_.400.png"))
```
...
subset_06_table
subset_06_table |>  tab_options(table.width = pct(65)) |> gtsave(here("z3t_r_l_heatmaps","2T_R_L_.600.png"))
```
...
subset_07_table
subset_07_table |>  tab_options(table.width = pct(65)) |> gtsave(here("z3t_r_l_heatmaps","2T_R_L_.700.png"))
```
...
subset_08_table
subset_08_table |>  tab_options(table.width = pct(65)) |> gtsave(here("z3t_r_l_heatmaps","2T_R_L_.800.png"))
```
