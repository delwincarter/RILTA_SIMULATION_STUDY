...
csv_directory <- here("7. 3T RILTA GEN RILTA ANALYZED")
...
#### Step 6.4, calculate the number of replications needed in the next round to get 500 keepable reps

```{r}
# Define min_max_table to get rows with the minimum and maximum violations
min_max_table <- violation_summary %>%
  filter(
    Total_Violations == max(Total_Violations) | Total_Violations == min(Total_Violations)
  ) %>%
  mutate(
    Row_Label = case_when(
      Total_Violations == max(Total_Violations) ~ "Maximum Violations",
      Total_Violations == min(Total_Violations) ~ "Minimum Violations"
    )
  )

# Correct the logic based on the calculation, including a cushion of 20 replications
recommended_reps <- violation_summary %>%
  summarise(
    Max_Violation_Percentage = max(Percentage_Violations),
    Max_Violations = max(Total_Violations)
  ) %>%
  mutate(
    # Calculate total additional violations based on max violations and percentage
    Additional_Runs = (500 + Max_Violations) * (Max_Violation_Percentage / 100),
    # Calculate total replications needed, including 20 extra for cushion
    Needed_Reps = ceiling(500 + Max_Violations + Additional_Runs + 20),  
    # Update the calculation note correctly
    Calculation = paste0("(500 + ", Max_Violations, ") + (", "(500 + ", Max_Violations, ") * ", Max_Violation_Percentage, "%) + 20 = ", round(Additional_Runs, 2), " additional runs")
  )

# Arrange the min and max violations in the desired order (min first, max second)
min_max_table_ordered <- min_max_table %>%
  arrange(match(Row_Label, c("Minimum Violations", "Maximum Violations")))

# Create the final table with labeled min/max rows in the correct order
final_summary_table <- min_max_table_ordered %>%
  select(
    Row_Label,  # Add the row label for min/max
    `Transition Probability` = Population,  # Rename Population to Transition Probability
    `Sample Size` = N,  # Rename N to Sample Size
    `Total Mplus Runs` = Total_Rows,  # Rename Total_Rows to Total Mplus Runs
    `Total Violations` = Total_Violations,  # Rename Total Violations to Total Violations
    `% of Violations` = Percentage_Violations  # Rename Percentage Violations to % of Violations
  ) %>%
  gt() %>%
  tab_header(
    title = "Monte Carlo Violation Summary",
    subtitle = "Highest and Lowest Violations"
  ) %>%
  cols_align(
    align = "center",  # Center all columns
    columns = everything()
  ...
  tab_footnote(
    footnote = md(
      paste0(
        "Recommended Reps for Next Run: ",
        recommended_reps$Calculation, ".\n\n",
        "**", recommended_reps$Needed_Reps, " Replications needed for followup MC study**"
      )
    ),
