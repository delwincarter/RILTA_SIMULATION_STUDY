...
output_folder <- "8_3T_RILTA_GEN_RILTA_ANALYZED"  # Updated folder name with _REP
...
# Step 2: Parallelized Processing
output_folder <- "8_3T_RILTA_GEN_RILTA_ANALYZED_REP"  # Updated folder name with _REP
file_list <- list.files(output_folder, pattern = "\\.out$", full.names = TRUE)
cluster_type <- ifelse(.Platform$OS.type == "windows", "PSOCK", "FORK")
num_cores <- detectCores() - 1  # Use all but one core
cl <- makeCluster(num_cores, type = cluster_type)
...
ggsave(here("3 Time Points", "zFIGURES", "x3t_rilta_rilta_plots", "plot_mover.svg"), plot = plot_mover_no_title, width = 6, height = 3, dpi = 300, device = "svg")
```
...
ggsave(here("3 Time Points", "zFIGURES", "x3t_rilta_rilta_plots", "plot_stayer.svg"), plot = plot_stayer_no_title, width = 6, height = 3, dpi = 300, device = "svg")
```
...
subset_02_table
subset_02_table |>  tab_options(table.width = pct(65)) |> gtsave(here("3 Time Points", "zHEATMAPS", "z3t_heatmaps","z2t_r_r_heatmaps", "2T_R_R_.200.png"))
```
...
subset_03_table
subset_03_table |>  tab_options(table.width = pct(65)) |> gtsave(here("3 Time Points", "zHEATMAPS", "z3t_heatmaps","z2t_r_r_heatmaps","2T_R_R_.300.png"))
```
...
subset_04_table
subset_04_table |>  tab_options(table.width = pct(65)) |> gtsave(here("3 Time Points", "zHEATMAPS", "z3t_heatmaps","z2t_r_r_heatmaps","2T_R_R_.400.png"))
```
...
subset_06_table
subset_06_table |>  tab_options(table.width = pct(65)) |> gtsave(here("3 Time Points", "zHEATMAPS", "z3t_heatmaps","z2t_r_r_heatmaps","2T_R_R_.600.png"))
```
...
subset_07_table
subset_07_table |>  tab_options(table.width = pct(65)) |> gtsave(here("3 Time Points", "zHEATMAPS", "z3t_heatmaps","z2t_r_r_heatmaps","2T_R_R_.700.png"))
```
...
subset_08_table
subset_08_table |>  tab_options(table.width = pct(65)) |> gtsave(here("3 Time Points", "zHEATMAPS", "z3t_heatmaps","z2t_r_r_heatmaps","2T_R_R_.800.png"))
```
