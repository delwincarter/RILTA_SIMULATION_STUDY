# Step 1: Define the function to process each 8-row chunk
process_chunk <- function(chunk_start, data) {
  if (chunk_start + 8 <= nrow(data)) {  # Ensure chunk is within bounds
    ...
    row8 <- data[chunk_start + 7, ]     # ll_csv
    
    # Extract relevant data
    rep_variable    <- row1[1]           # Rep number from row 1
    columns_1_to_10 <- row2[1:10]        # First 10 columns from row 2
    trans11         <- row3[6]           # TRANS11 (column 7 from row 4)
    se11            <- row6[6]           # SE_11 (column 7 from row 7)
    ll_csv          <- row8[2]           # ll_csv (column 2 from row 8)
...
# Step 6: Set up the sequence of chunk starts (every 8 rows)
