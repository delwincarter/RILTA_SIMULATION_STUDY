...
M3Ac1u <- c(0.9933, 0.915, 0.7998, 0.990, 0.990)
M3Ac2u <- c(0.650, 0.692, 0.694, 0.570, 0.800)
...
process_row_parallel <- function(i, data) {
  ...
  combined_row <- cbind(data[i, ], as.data.frame(t(actual_values)))  # Add the actual values to the row
  return(combined_row)
}
...
final_combined_data_with_trans_se <- final_combined_data %>%
  dplyr::select(FileName, Replication, TRANS11, SE_11, ll_csv) %>%
