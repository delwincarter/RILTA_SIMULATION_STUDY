...
format:
  html:
    ...
    fig-format: svg
    font:
      main: "Avenir Next LT Pro, sans-serif"
    page-layout: full
    code-tools: true
...
# Take a random sample of 1 violator
set.seed(123)
sample_size <- 250
...
plot_violator <- function(i) {
  ...
  # Create labels for legend with probabilities
labels <- c(
  paste0("Population Probabilities Class 1: (", round(row_data$Ac1u1, 3), ", ", round(row_data$Ac1u2, 3), ", ", round(row_data$Ac1u3, 3), ", ", round(row_data$Ac1u4, 3), ", ", round(row_data$Ac1u5, 3), ")"),
  paste0("Population Probabilities Class 2: (", round(row_data$Ac2u1, 3), ", ", round(row_data$Ac2u2, 3), ", ", round(row_data$Ac2u3, 3), ", ", round(row_data$Ac2u4, 3), ", ", round(row_data$Ac2u5, 3), ")"),
  paste0("Estimated Probabilities Class 1: (", round(row_data$Ec1u1, 3), ", ", round(row_data$Ec1u2, 3), ", ", round(row_data$Ec1u3, 3), ", ", round(row_data$Ec1u4, 3), ", ", round(row_data$Ec1u5, 3), ")"),
  paste0("Estimated Probabilities Class 2: (", round(row_data$Ec2u1, 3), ", ", round(row_data$Ec2u2, 3), ", ", round(row_data$Ec2u3, 3), ", ", round(row_data$Ec2u4, 3), ", ", round(row_data$Ec2u5, 3), ")")
)

  # Create data frame for plotting
plot_data <- data.frame(
  Items = rep(1:5, 4),
  Probabilities = c(actual_values, estimated_probabilities),
  Class = factor(rep(labels, each = 5), levels = labels)
)
...
p <- ggplot(plot_data, aes(x = Items, y = Probabilities, color = Class, group = Class)) +
  geom_line(size = 1.2) +
  geom_point(size = 3) +
  labs(title = paste0(file_name, " (Violator, Replication ", row_data$Replication, ")"), x = "Items", y = "Probabilities") +
  ...
  scale_x_continuous(limits = c(0, 5.5), breaks = 1:5, labels = expression(italic(u[1]), italic(u[2]), italic(u[3]), italic(u[4]), italic(u[5])), expand = expansion(mult = c(0, 0.1)))
...
# Take a random sample of 1 non-violator
set.seed(123)  # For reproducibility
non_violators <- filter(final_data_with_actuals, Any_Violation == 0)
sample_size_non_violators <- 250
...
p <- ggplot(plot_data, aes(x = Items, y = Probabilities, color = Class, group = Class)) +
  geom_line(size = 1.2) +
  geom_point(size = 3) +
  labs(title = paste0(file_name, " (Non-Violator, Replication ", row_data$Replication, ")"), x = "Items", y = "Probabilities") +
  theme_minimal(base_size = 18) +
  theme(
    text = element_text(family = "Avenir Next", color = "black"),
    plot.title = element_text(size = 18, hjust = 0.5),
...
# Write CSV
write.csv(violation_summary, here("Simulations", "STUDY_1", "2 Time Points", 'zdata', "violation_summary_l_l_2t.csv"), row.names = FALSE)
```
...
# Write CSV
write.csv(violation_summary2, here("Simulations", "STUDY_1", "2 Time Points", 'zdata', "violation_summary2_l_l_2t.csv"), row.names = FALSE)
```
...
# Write CSV
write.csv(violation_summary_final, here("Simulations", "STUDY_1", "2 Time Points", 'zdata', "violation_summary_final_l_l_2t.csv"), row.names = FALSE)
```
...
violation_summary_final <- read.csv(
  here("Simulations", "STUDY_1", "2 Time Points", "zdata", "violation_summary_final_l_l_2t.csv")
)
...
invisible(ggsave(
  filename = here("Simulations", "STUDY_1", "2 Time Points", "zposthoc", "l_l_2t_violation_visuals", "l_l_2t_violation_plot.svg"),
  plot = violation_plot,
  width = 7,
  height = 5,
  units = "in",
...
invisible(save_as_image(
  ...
  path = here("Simulations", "STUDY_1", "2 Time Points", "zposthoc", "l_l_2t_violation_visuals", "l_l_2t_violation_pairwise.svg")
))
...
invisible(ggsave(
  filename = here("Simulations", "STUDY_1", "2 Time Points", "zposthoc", "l_l_2t_bias_visuals", "l_l_2t_ANOVA_BIAS_plot.svg"),
  plot = bias_plot,
  width = 7,
  height = 5,
  units = "in",
...
invisible(save_as_image(
  ...
  path = here("Simulations", "STUDY_1", "2 Time Points", "zposthoc", "l_l_2t_bias_visuals", "l_l_2t_pairwise_bias_population.svg")
))
...
gt_table |> tab_options(table.width = pct(75)) |> gtsave(here("Simulations", "STUDY_1", "2 Time Points", "zHEATMAPS", "z2t_l_l_heatmaps", "2T_L_L.png")
)
