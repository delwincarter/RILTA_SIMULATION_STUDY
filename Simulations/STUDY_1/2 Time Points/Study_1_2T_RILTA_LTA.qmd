...
subtitle: "Study 1.3: RILTA Generated, LTA Analyzed with Two Timepoints"
...
## Study 1.3: RILTA Generated, LTA Analyzed
...
rilta_lta_func <- function(N, TPs, TH, lambda) {
  ...
  RILTA_LTA_Model<- mplusModeler(RILTA_LTA, 
                                   dataout = here('Simulations', 'STUDY_1', '2 Time Points', "3_2T_RILTA_GEN_LTA_ANALYZED", glue("RILTA_LTA_N_{N}_TP_{TPs}_TH_{TH}_lambda_{lambda}.dat")),
                                   modelout = glue(here('Simulations', 'STUDY_1', '2 Time Points', "3_2T_RILTA_GEN_LTA_ANALYZED", "RILTA_LTA_N_{N}_TP_{TPs}_TH_{TH}_lambda_{lambda}.inp")),
...
```{r}
#| label: "Visualize-ll-differences2"
#| echo: true
#| message: false
#| warning: false
#| code-fold: false
...
```{r}
#| label: "create-LL-DF-for-inspection"
#| echo: true
#| message: false
#| warning: false
#| code-fold: false
...
# Write CSV
write.csv(violation_summary, here("Simulations", "STUDY_1", "2 Time Points", 'zdata', "violation_summary_r_l_2t.csv"), row.names = FALSE)
```
...
# READ CSV
violation_summary <- read.csv(here("Simulations", "STUDY_1", "2 Time Points", 'zdata', "violation_summary_r_l_2t.csv"))
...
invisible(save_as_image(ft_200, here("Simulations", "STUDY_1", "2 Time Points", "zErrors",  "r_l_e&v_200_1.svg")))
invisible(save_as_image(ft_300, here("Simulations", "STUDY_1", "2 Time Points", "zErrors", "r_l_e&v_300_1.svg")))
invisible(save_as_image(ft_400, here("Simulations", "STUDY_1", "2 Time Points", "zErrors", "r_l_e&v_400_1.svg")))
invisible(save_as_image(ft_600, here("Simulations", "STUDY_1", "2 Time Points", "zErrors", "r_l_e&v_600_1.svg")))
invisible(save_as_image(ft_700, here("Simulations", "STUDY_1", "2 Time Points", "zErrors", "r_l_e&v_700_1.svg")))
invisible(save_as_image(ft_800, here("Simulations", "STUDY_1", "2 Time Points", "zErrors", "r_l_e&v_800_1.svg")))
...
rilta_lta_func <- function(N_numeric, TPs, Lambda, Adjusted_Replications_Needed) {
  ...
  RILTA_LTA <- mplusObject(
   ...
   # Run Mplus model
  RILTA_LTA_Model <- mplusModeler(RILTA_LTA, 
                                   dataout = here('Simulations', 'STUDY_1', '2 Time Points', "3_2T_RILTA_GEN_LTA_ANALYZED_REP", glue("RILTA_LTA_N_{N_numeric}_TP_{TPs}_TH_1_Lambda_{Lambda}.dat")),
                                   modelout = here('Simulations', 'STUDY_1', '2 Time Points', "3_2T_RILTA_GEN_LTA_ANALYZED_REP", glue("RILTA_LTA_N_{N_numeric}_TP_{TPs}_TH_1_Lambda_{Lambda}.inp")),
                                   check = TRUE, run = TRUE, hashfilename = FALSE)
...
invisible(cl <- makeCluster(num_cores, type = cluster_type))
# Export necessary objects to the cluster
invisible(clusterExport(cl, c("rilta_lta_func", "violation_summary2", "here", "glue", "mplusModeler", "mplusObject")))
...
# Write CSV
write.csv(violation_summary2, here("Simulations", "STUDY_1", "2 Time Points", 'zdata', "violation_summary2_r_l_2t.csv"), row.names = FALSE)

```

Prepare Subsets for Vizualization
...
violation_summary2 <- read.csv(here("Simulations", "STUDY_1", "2 Time Points", "zdata", "violation_summary2_r_l_2t.csv"))
...
# 🛠 Now subset properly — no factors!
...
# Step 1: Preprocess the second summary
violation_summary2 <- violation_summary2 %>%
  mutate(
    Lambda     = as.numeric(Lambda),
    Population = as.character(Population),
    N_numeric  = as.numeric(N_numeric)
  ) %>%
  rename_with(~ paste0(.x, "_2"), 
              -c(Population, N, N_numeric, Lambda))

# Step 2: Merge and format final summary
...
# Step 3: Save to CSV
write.csv(
  violation_summary_final,
  here("Simulations", "STUDY_1", "2 Time Points", "zdata", "violation_summary_final_r_l_2t.csv"),
  row.names = FALSE
)
...
Prepare Subsets for Fnal Violations
...
# Subset populations manually from violation_summary_final
subset_200 <- violation_summary_final %>% filter(Population == ".200")
subset_300 <- violation_summary_final %>% filter(Population == ".300")
subset_400 <- violation_summary_final %>% filter(Population == ".400")
subset_600 <- violation_summary_final %>% filter(Population == ".600")
subset_700 <- violation_summary_final %>% filter(Population == ".700")
subset_800 <- violation_summary_final %>% filter(Population == ".800")

# Store subsets manually in a list
transition_subsets <- list(
  "TP_200" = subset_200,
  "TP_300" = subset_300,
  "TP_400" = subset_400,
...
if (!is.null(title)) {
  ft <- add_header_lines(
    ...
    fontsize(i = 1, size = 13, part = "header")  # ✅ removed bold
}

# ⬇️ NOW apply font to ALL parts including the new header line
ft <- font(ft, fontname = "Avenir Next", part = "all")
...
# Save each flextable as an SVG file
invisible(save_as_image(ft_200, here("Simulations", "STUDY_1", "2 Time Points", "zErrors", "zr_l_e&v_200_FINAL.svg")))
invisible(save_as_image(ft_300, here("Simulations", "STUDY_1", "2 Time Points", "zErrors", "zr_l_e&v_300_FINAL.svg")))
invisible(save_as_image(ft_400, here("Simulations", "STUDY_1", "2 Time Points", "zErrors", "zr_l_e&v_400_FINAL.svg")))
invisible(save_as_image(ft_600, here("Simulations", "STUDY_1", "2 Time Points", "zErrors", "zr_l_e&v_600_FINAL.svg")))
invisible(save_as_image(ft_700, here("Simulations", "STUDY_1", "2 Time Points", "zErrors", "zr_l_e&v_700_FINAL.svg")))
invisible(save_as_image(ft_800, here("Simulations", "STUDY_1", "2 Time Points", "zErrors", "zr_l_e&v_800_FINAL.svg")))
...
violation_summary_final <- read.csv(
  ...
  mutate(
    ...
    Lambda = factor(
      ...
      labels = c("λ = 0", "λ = 0.2", "λ = 0.4", "λ = 0.6", "λ = 0.8", "λ = 1")
    ),
    # Create Lambda_label with formatted strings for italicized λ
    Lambda_label = factor(
      sapply(Lambda, function(x) {
        value <- sub("^λ\\s*", "", as.character(x))  # Extract "= 0", "= 0.2", etc.
        paste0("italic(λ) * '", value, "'")  # Format as "italic(λ) * '= 0'"
      }),
      levels = sapply(c("λ = 0", "λ = 0.2", "λ = 0.4", "λ = 0.6", "λ = 0.8", "λ = 1"), function(x) {
        value <- sub("^λ\\s*", "", x)
        paste0("italic(λ) * '", value, "'")
      })
    )
  ) %>%
  droplevels()
...
invisible(ggsave(
  filename = here("Simulations", "STUDY_1", "2 Time Points", "zposthoc", "r_l_2t_violation_visuals", "l_r_2t_violation_plot.svg"),
  plot = violation_plot,
  width = 7,
  height = 5,
  units = "in",
  ...
  df <- merged_table %>%
    ...
    dplyr::mutate(
      ...
      Effect = dplyr::case_when(
        ...
        term == "Population" ~ "T₁₁",
        term == "Lambda" ~ "λ",
        term == "N_numeric:Population" ~ "N:T₁₁",
        term == "N_numeric:Lambda" ~ "N:λ",
        term == "Population:Lambda" ~ "T₁₁:λ",
        TRUE ~ term
      )
    ) %>%
...
anova_results <- dplyr::bind_rows(
  extract_anova_results(anova_main_type3, "Main Effects"),
  extract_anova_results(anova_np, "N × T₁₁"),
  extract_anova_results(anova_nl, "N × λ"),
  extract_anova_results(anova_pl, "T₁₁ × λ")
)
...
# Create table
violation_anova_table <- flextable(anova_results_fmt) %>%
  set_caption("ANOVA Results for Final True Violation Rate") %>%
  set_header_labels(
    ANOVA = "ANOVA",
    Effect = "Effect",
    df = "df",
  ...
  compose(part = "body", j = "Effect", i = ~ Effect == "T₁₁", value = as_paragraph(as_i("T"), as_sub("11"))) %>%
  compose(part = "body", j = "Effect", i = ~ Effect == "λ", value = as_paragraph(as_i("λ"))) %>%
  compose(part = "body", j = "Effect", i = ~ Effect == "N:T₁₁", value = as_paragraph(as_i("N"), ":", as_i("T"), as_sub("11"))) %>%
  compose(part = "body", j = "Effect", i = ~ Effect == "N:λ", value = as_paragraph(as_i("N"), ":", as_i("λ"))) %>%
  compose(part = "body", j = "Effect", i = ~ Effect == "T₁₁:λ", value = as_paragraph(as_i("T"), as_sub("11"), ":", as_i("λ"))) %>%
  ...
  compose(part = "body", j = "ANOVA", i = ~ ANOVA == "N × T₁₁", value = as_paragraph(as_i("N"), " × ", as_i("T"), as_sub("11"))) %>%
  compose(part = "body", j = "ANOVA", i = ~ ANOVA == "N × λ", value = as_paragraph(as_i("N"), " × ", as_i("λ"))) %>%
  compose(part = "body", j = "ANOVA", i = ~ ANOVA == "T₁₁ × λ", value = as_paragraph(as_i("T"), as_sub("11"), " × ", as_i("λ"))) %>%
  ...
  bg(i = ~ ANOVA %in% c("Main Effects", "N × λ"), bg = "#f0f0f0", part = "body") %>%
  border_remove() %>%
...
invisible(flextable::save_as_image(violation_anova_table, path = here::here("Simulations", "STUDY_1", "2 Time Points", "zposthoc", "r_l_2t_violation_visuals", "r_l_2t_violation_anova_table.svg")))
```

**Create Violation Pairwise Comparisons Table**
...
format_pairwise_ft <- function(df, group_label) {
  ...
  left_right <- stringr::str_match(pairwise_clean$contrast, "Population\\.(\\d{3}) - Population\\.(\\d{3})")
...
Create Lambda Violation Pairwise Tables
...
# Pairwise comparisons for Population within each Lambda level
all_comparisons <- summary(emmeans(aov_pl, pairwise ~ Population | Lambda)$contrasts, infer = TRUE)

# Split into list by Lambda
pairwise_list <- split(all_comparisons, all_comparisons$Lambda)

# Create and print each Lambda-specific table
flextable_list <- lapply(names(pairwise_list), function(lam) {
  format_pairwise_ft(pairwise_list[[lam]], lam)
})
...
Create N Violation Pairwise Tables

```{r}
#| label: "pairwise-comparisons-sample-size"
#| echo: true
#| message: false
#| warning: false

# Pairwise comparisons for Population within each N_numeric level using aov_np
sample_comparisons <- summary(emmeans(aov_np, pairwise ~ Population | N_numeric)$contrasts, infer = TRUE)

# Split into list by Sample Size
pairwise_list_n <- split(sample_comparisons, sample_comparisons$N_numeric)

# Create and print each N-specific table with styled header label
flextable_list_n <- lapply(names(pairwise_list_n), function(n_size) {
  format_pairwise_ft(
    pairwise_list_n[[n_size]],
    as_paragraph(as_i("N"), " = ", n_size)
  )
})
...
invisible(save_as_image(flextable_list[[1]], path = here("Simulations", "STUDY_1", "2 Time Points", "zposthoc", "r_l_2t_violation_visuals", "r_l_2t_pairwise_lambda_0.svg")))
invisible(save_as_image(flextable_list[[2]], path = here("Simulations", "STUDY_1", "2 Time Points", "zposthoc", "r_l_2t_violation_visuals", "r_l_2t_pairwise_lambda_0.2.svg")))
invisible(save_as_image(flextable_list[[3]], path = here("Simulations", "STUDY_1", "2 Time Points", "zposthoc", "r_l_2t_violation_visuals", "r_l_2t_pairwise_lambda_0.4.svg")))
invisible(save_as_image(flextable_list[[4]], path = here("Simulations", "STUDY_1", "2 Time Points", "zposthoc", "r_l_2t_violation_visuals", "r_l_2t_pairwise_lambda_0.6.svg")))
invisible(save_as_image(flextable_list[[5]], path = here("Simulations", "STUDY_1", "2 Time Points", "zposthoc", "r_l_2t_violation_visuals", "r_l_2t_pairwise_lambda_0.8.svg")))
invisible(save_as_image(flextable_list[[6]], path = here("Simulations", "STUDY_1", "2 Time Points", "zposthoc", "r_l_2t_violation_visuals", "r_l_2t_pairwise_lambda_1.svg")))

# Save tables by N size
invisible(save_as_image(flextable_list_n[[1]], path = here("Simulations", "STUDY_1", "2 Time Points", "zposthoc", "r_l_2t_violation_visuals", "r_l_2t_pairwise_n_500.svg")))
invisible(save_as_image(flextable_list_n[[2]], path = here("Simulations", "STUDY_1", "2 Time Points", "zposthoc", "r_l_2t_violation_visuals", "r_l_2t_pairwise_n_1000.svg")))
invisible(save_as_image(flextable_list_n[[3]], path = here("Simulations", "STUDY_1", "2 Time Points", "zposthoc", "r_l_2t_violation_visuals", "r_l_2t_pairwise_n_2000.svg")))
invisible(save_as_image(flextable_list_n[[4]], path = here("Simulations", "STUDY_1", "2 Time Points", "zposthoc", "r_l_2t_violation_visuals", "r_l_2t_pairwise_n_4000.svg")))
...
calc_mc_values <- function(data) {
  ...
  mc_values <- cleaned_data %>%
    group_by(Population, Lambda, N) %>%  # ✅ Include Lambda in grouping
    summarize(
      group_size   = n(),
  ...
  mc_values <- cleaned_data %>%
    ...
    distinct() %>%
    right_join(mc_values, by = c("Population", "Lambda", "N"))  # ✅ Join by Population, Lambda, and N
...
bootstrap_mc_values <- function(data, n_bootstrap, sample_size) {
  ...
  grouped_data <- data %>%
    group_by(Population, Lambda, N, Transitions) %>%  # ✅ Include Lambda in grouping
...
final_mc_values <- original_mc_values %>%
  left_join(bootstrap_aggregates, by = c("Population", "Lambda", "N", "Transitions")) %>%  # ✅ Join by Population, Lambda, and N
  mutate(
...
# Save results
write.csv(final_mc_values, here("Simulations", "STUDY_1", "2 Time Points", "zbootstrapping", "r_l_2t_mc_final_mc_values.csv"), row.names = FALSE)
write.csv(boot_results, here("Simulations", "STUDY_1", "2 Time Points", "zbootstrapping", "r_l_2t_boot_results.csv"), row.names = FALSE)
...
final_mc_values <- read.csv(
  here("Simulations", "STUDY_1", "2 Time Points", "zbootstrapping", "r_l_2t_mc_final_mc_values.csv")
) %>%
  ...
  mutate(
    ...
    Lambda = factor(Lambda, labels = c("λ = 0", "λ = 0.2", "λ = 0.4", "λ = 0.6", "λ = 0.8", "λ = 1"))
  ) %>%
  droplevels()
...
invisible(ggsave(
  filename = here("Simulations", "STUDY_1", "2 Time Points", "zposthoc", "r_l_2t_bias_visuals", "r_l_2t_ANOVA_BIAS_plot.svg"),
  plot = bias_plot,
  width = 7,
  height = 5,
  units = "in",
...
Save Bias ANOVA table

```{r}
# Save the anova_table as an SVG in the correct directory
invisible(flextable::save_as_image(bias_anova_table, path = here::here("Simulations", "STUDY_1", "2 Time Points", "zposthoc", "r_l_2t_bias_visuals", "r_l_2t_bias_anova_table.svg")))
```
...
Create Bias Lambda Pairwise Tables
...
lambda_levels <- c("λ = 0", "λ = 0.2", "λ = 0.4", "λ = 0.6", "λ = 0.8", "λ = 1")
...
flextable_list <- lapply(names(pairwise_list), function(lam) {
  group_label <- lam  # Just use the Lambda value (e.g., "λ = 0")
...
# Save tables by lambda
invisible(save_as_image(flextable_list[[1]], path = here("Simulations", "STUDY_1", "2 Time Points", "zposthoc", "r_l_2t_bias_visuals", "r_l_2t_pairwise_bias_lambda_0.svg")))
invisible(save_as_image(flextable_list[[2]], path = here("Simulations", "STUDY_1", "2 Time Points", "zposthoc", "r_l_2t_bias_visuals", "r_l_2t_pairwise_bias_lambda_0.2.svg")))
invisible(save_as_image(flextable_list[[3]], path = here("Simulations", "STUDY_1", "2 Time Points", "zposthoc", "r_l_2t_bias_visuals", "r_l_2t_pairwise_bias_lambda_0.4.svg")))
invisible(save_as_image(flextable_list[[4]], path = here("Simulations", "STUDY_1", "2 Time Points", "zposthoc", "r_l_2t_bias_visuals", "r_l_2t_pairwise_bias_lambda_0.6.svg")))
invisible(save_as_image(flextable_list[[5]], path = here("Simulations", "STUDY_1", "2 Time Points", "zposthoc", "r_l_2t_bias_visuals", "r_l_2t_pairwise_bias_lambda_0.8.svg")))
invisible(save_as_image(flextable_list[[6]], path = here("Simulations", "STUDY_1", "2 Time Points", "zposthoc", "r_l_2t_bias_visuals", "r_l_2t_pairwise_bias_lambda_1.svg")))
...
final_mc_values <- read.csv(here("Simulations", "STUDY_1", "2 Time Points", "zbootstrapping", "r_l_2t_mc_final_mc_values.csv"))
...
ggsave(here('Simulations', 'STUDY_1', "2 Time Points", "zFigures", "x2t_rilta_lta_plots", "plot_mover.svg"), plot = plot_mover_no_title, width = 6, height = 4, dpi = 300, device = "svg")
```
...
ggsave(here('Simulations', 'STUDY_1', "2 Time Points", "zFigures", "x2t_rilta_lta_plots", "plot_stayer.svg"), plot = plot_stayer_no_title, width = 6, height = 4, dpi = 300, device = "svg")
```
...
create_table <- function(subset, transition_probability) {
  ...
  gt_table <- subset %>%
    ...
    tab_row_group(label = "Lambda RI Loading of 0 (λ)",  rows =  1: 4) %>%
    tab_row_group(label = "Lambda RI Loading of .2 (λ)", rows =  5: 8) %>%
    tab_row_group(label = "Lambda RI Loading of .4 (λ)", rows =  9:12) %>%
    tab_row_group(label = "Lambda RI Loading of .6 (λ)", rows = 13:16) %>%
    tab_row_group(label = "Lambda RI Loading of .8 (λ)", rows = 17:20) %>%
    tab_row_group(label = "Lambda RI Loading of 1 (λ)", rows = 21:24) %>%
    tab_style(
      style     = cell_text(weight = "normal", style = "italic"),
      locations = cells_row_groups()
    ) %>%
    fmt_number(columns = c("Parameter_Bias","SE_Bias"), decimals = 2) %>%
    fmt_number(columns = "Power", decimals = 3) %>%
text_transform(
  ...
  fn = function(x) sub("^0\\.", ".", x)
    ...
    tab_options(
      ...
      # **fix the two stray grey lines**: make table‐body borders 1px black
    ...
    # ensure the top border above each Lambda‑RI group is black 1px
    tab_style(
...
subset_02_table |> 
  ...
  gtsave(here('Simulations', 'STUDY_1', "2 Time Points", "zHEATMAPS", "z2t_r_l_heatmaps", "2T_R_L_.200.png"))
```
...
subset_03_table
subset_03_table |>  tab_options(table.width = pct(75)) |> gtsave(here("Simulations", "STUDY_1", "2 Time Points", "zHEATMAPS", "z2t_r_l_heatmaps","2t_r_l_.300.png"))
```
...
subset_04_table
subset_04_table |>  tab_options(table.width = pct(75)) |> gtsave(here("Simulations", "STUDY_1", "2 Time Points", "zHEATMAPS", "z2t_r_l_heatmaps","2t_r_l_.400.png"))
```
...
subset_06_table
subset_06_table |>  tab_options(table.width = pct(75)) |> gtsave(here("Simulations", "STUDY_1", "2 Time Points", "zHEATMAPS", "z2t_r_l_heatmaps","2t_r_l_.600.png"))
```
...
subset_07_table
subset_07_table |>  tab_options(table.width = pct(75)) |> gtsave(here("Simulations", "STUDY_1", "2 Time Points", "zHEATMAPS", "z2t_r_l_heatmaps","2t_r_l_.700.png"))
```
...
subset_08_table
subset_08_table |>  tab_options(table.width = pct(75)) |> gtsave(here("Simulations", "STUDY_1", "2 Time Points", "zHEATMAPS", "z2t_r_l_heatmaps","2t_r_l_.800.png"))
```
