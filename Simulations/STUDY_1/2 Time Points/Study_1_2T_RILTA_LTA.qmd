...
# Laptop: 48 conditions (N_numeric = 4000 all TPs + N_numeric = 2000 TPs 1.385, 0.85), 11 cores available
violation_summary <- violation_summary[(violation_summary$N_numeric == 4000) | 
                                       (violation_summary$N_numeric == 2000 & violation_summary$TPs %in% c(1.385, 0.85)), ]

# Old Rig 1: 24 conditions (N_numeric = 500, TPs = 1.385, 0.85, 0.41, -0.41), 5 cores available
#violation_summary <- violation_summary[(violation_summary$N_numeric == 500 & violation_summary$TPs %in% c(1.385, 0.85, 0.41, -0.41)), ]

# OldRig 2: 24 conditions (N_numeric = 500, TPs = -0.85, -1.385 + N_numeric = 2000, TPs = -0.85, -1.385), 5 cores available
#violation_summary <- violation_summary[(violation_summary$N_numeric == 500 & violation_summary$TPs %in% c(-0.85, -1.385)) | 
#                                       (violation_summary$N_numeric == 2000 & violation_summary$TPs %in% c(-0.85, -1.385)), ]

# Old Rig 3: 24 conditions (N_numeric = 1000, TPs = 1.385, 0.85, 0.41 + N_numeric = 2000, TPs = -0.41), 5 cores available
...
# Old Rig 4: 24 conditions (N_numeric = 1000, TPs = -0.41, -0.85, -1.385 + N_numeric = 2000, TPs = 0.41), 5 cores available
...
invisible(clusterExport(cl, c("rilta_lta_func", "violation_summary", "here", "glue", "mplusModeler", "mplusObject")))
...
calc_mc_values <- function(data) {
  ...
  mc_values <- cleaned_data %>%
    select(FileName, Population, Lambda, N, Transitions) %>%
    distinct() %>%
  ...
  if (any(!(subset$Parameter_Bias >= -9.99 & subset$Parameter_Bias <= 9.99), na.rm = TRUE)) {
    gt_table <- gt_table %>%
      data_color(
        ...
        domain = c(-160, 160)  # Adjust the domain to reflect the range of values
      ) %>%
      tab_footnote(
...
subset_02_table |> 
  ...
  gtsave(here("2 Time Points", "zHEATMAPS", "z2t_heatmaps", "z2t_r_l_heatmaps", "2T_R_L_.200.png"))
```
...
subset_03_table
subset_03_table |>  tab_options(table.width = pct(65)) |> gtsave(here("2 Time Points", "zHEATMAPS", "z2t_heatmaps", "z2t_r_l_heatmaps","2T_R_L_.300.png"))
```
...
subset_04_table
subset_04_table |>  tab_options(table.width = pct(65)) |> gtsave(here("2 Time Points", "zHEATMAPS", "z2t_heatmaps", "z2t_r_l_heatmaps","2T_R_L_.400.png"))
```
...
subset_06_table
subset_06_table |>  tab_options(table.width = pct(65)) |> gtsave(here("2 Time Points", "zHEATMAPS", "z2t_heatmaps", "z2t_r_l_heatmaps","2T_R_L_.600.png"))
```
...
subset_07_table
subset_07_table |>  tab_options(table.width = pct(65)) |> gtsave(here("2 Time Points", "zHEATMAPS", "z2t_heatmaps", "z2t_r_l_heatmaps","2T_R_L_.700.png"))
```
