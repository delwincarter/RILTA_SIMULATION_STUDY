...
subtitle: "Study 1.5: LTA Generated, LTA Analyzed with Three Timepoints"
...
::: {layout-ncol="2"}
![](images/LVG%20FINAL-01.png){width="300"}
...
![](images/clipboard-2674798363.png){width="351"}
...
#p11 <- p1 %>% 
  #filter(N_numeric == 500)

#p12 <- p1 %>% 
  #filter(N_numeric == 1000)

#p13 <- p1 %>% 
 #filter(N_numeric == 2000)
...
plot_violator <- function(i) {
  ...
  ggsave(filename = file.path(here('Simulations', 'STUDY_1', "3 Time Points", "zViolator_Plots","z3t_lta_lta_violator_plots", paste0("violator_plot_", i, "_", file_name, ".png"))),
...
# Write CSV
write.csv(violation_summary, here("Simulations", "STUDY_1", "3 Time Points", 'zdata', "violation_summary_l_l_3t.csv"), row.names = FALSE)


```

```{r}
violation_summary <- read.csv(
  here("Simulations", "STUDY_1", "3 Time Points", "zdata", "violation_summary_l_l_3t.csv"),
  check.names = FALSE
) %>%
  rename_with(trimws)  # ✅ Trim any leading/trailing spaces from col names
...
#| warning: false
#| 
violation_summary <- read.csv(here("Simulations", "STUDY_1", "3 Time Points", 'zdata', "violation_summary_l_l_3t.csv"))
...
invisible(save_as_image(violation_summary_table, path = here('Simulations', 'STUDY_1', "3 Time Points", "zErrors", "3t_l_l_e&v_1.svg"))
   )
```
...
lta_lta_func <- function(N_numeric, TPs, Adjusted_Replications_Needed) {
   ...
   LTA_LTA <- mplusObject(
    TITLE = glue("Generate LTA_LTA_N_{N_numeric}_TP_{TPs}"),
    ...
    MONTECARLO =
      ...
      !!SAVE = repM1*.dat;
      RESULTS = LTA_LTA_N_{N_numeric}_TP_{TPs}.csv;"),
  ...
  # Run Mplus model
  LTA_LTA_Model<- mplusModeler(LTA_LTA, 
                                   dataout = here('Simulations', 'STUDY_1', '3 Time Points', '5_3T_LTA_GEN_LTA_ANALYZED_REP', glue("LTA_LTA_N_{N_numeric}_TP_{TPs}.dat")),
                                   modelout = glue(here('Simulations', 'STUDY_1', '3 Time Points', '5_3T_LTA_GEN_LTA_ANALYZED_REP', "LTA_LTA_N_{N_numeric}_TP_{TPs}.inp")),
                                   check = TRUE, run = TRUE, hashfilename = FALSE)
return(LTA_LTA_Model)
}
...
write.csv(violation_summary2, here("Simulations", "STUDY_1", "3 Time Points", "zdata", "violation_summary2_l_l_3t.csv"), row.names = FALSE)
```
...
violation_summary2 <- read.csv(here("Simulations", "STUDY_1", "3 Time Points", "zdata", "violation_summary2_l_l_3t.csv"))
...
invisible(save_as_image(violation_summary_table, path = here('Simulations', 'STUDY_1', "3 Time Points", "zErrors", "z3t_l_l_e&v_2L.svg"))
   )
```
...
write.csv(violation_summary_final, here("Simulations", "STUDY_1", "3 Time Points", "zdata", "violation_summary_final_l_l_3t.csv"), row.names = FALSE)
...
violation_summary_final <- read.csv(here("Simulations", "STUDY_1", "3 Time Points", "zdata", "violation_summary_final_l_l_3t.csv"))
...
invisible(save_as_image(violation_summary_final_table, path = here('Simulations', 'STUDY_1', "3 Time Points", "zErrors", "zl_l_e&v_FINAL.svg")
   ))
```
...
violation_summary_final <- read.csv(
  here("Simulations", "STUDY_1", "3 Time Points", "zdata", "violation_summary_final_l_l_3t.csv")
) %>%
...
invisible(ggsave(
  filename = here("Simulations", "STUDY_1", "3 Time Points", "zposthoc", "l_l_3t_violation_visuals", "l_l_3t_violation_plot.svg"),
  plot = violation_plot,
  width = 7,
  height = 5,
  units = "in",
...
invisible(save_as_image(
  ...
  path = here("Simulations", "STUDY_1", "3 Time Points", "zposthoc", "l_l_3t_violation_visuals", "l_l_3t_pairwise.svg")
))
...
# Save results
write.csv(final_mc_values, here("Simulations", "STUDY_1", "3 Time Points", "zbootstrapping", "l_l_3t_mc_final_mc_values.csv"), row.names = FALSE)
write.csv(boot_results, here("Simulations", "STUDY_1", "3 Time Points", "zbootstrapping", "l_l_3t_boot_results.csv"), row.names = FALSE)
...
final_mc_values <- read.csv(here("Simulations", "STUDY_1", "3 Time Points", "zbootstrapping", "l_l_3t_mc_final_mc_values.csv")) %>%
...
anova_bias <- aov(Parameter_Bias ~ N_numeric + Population, data = final_mc_values)
anova_bias_type3 <- Anova(anova_bias, type = "III") # MODIFIED: Compute Type 3 SS
print(anova_bias_type3) # MODIFIED: Print Type 3 ANOVA

# Calculate effect sizes (partial eta-squared)
print(eta_squared(anova_bias_type3, partial = TRUE)) # MODIFIED: Use Type 3 output for main effects only
...
bias_plot <- ggplot(final_mc_values, aes(x = Population, y = Parameter_Bias, fill = N)) +
  ...
  scale_y_continuous(limits = c(-5, 5)) +  # 🔥 Tighter y-axis for better visibility
  theme_minimal(base_size = 12) +
  theme(
...
invisible(ggsave(
  filename = here("Simulations", "STUDY_1", "3 Time Points", "zposthoc", "l_l_3t_bias_visuals", "l_l_3t_ANOVA_BIAS_plot.svg"),
  plot = bias_plot,
  width = 7,
  height = 5,
  units = "in",
...
#| warning: false
invisible(save_as_image(
  ft_bias,  # your flextable object
  path = here("Simulations", "STUDY_1", "3 Time Points", "zposthoc", "l_l_3t_bias_visuals", "l_l_3t_pairwise_bias.svg")
))
...
ggsave(here('Simulations', 'STUDY_1', "3 Time Points", "zFIGURES", "x3t_lta_lta_plots", "plot_mover.svg"), plot = plot_mover_no_title, width = 6, height = 3, dpi = 300, device = "svg")
```
...
ggsave(here('Simulations', 'STUDY_1', "3 Time Points", "zFigures", "x3t_lta_lta_plots", "plot_stayer.svg"), plot = plot_stayer_no_title, width = 6, height = 3, dpi = 300, device = "svg")
```
...
# Step 4: dplyr::select necessary columns for the final display
