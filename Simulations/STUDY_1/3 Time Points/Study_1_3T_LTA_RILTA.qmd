...
create_flextable <- function(data) {
  ...
  data <- data %>%
    select(
      N_numeric,  
      Population,  # Keep as "Population", not "T11"
      Total_Rows,
      Total_Violations,
  ...
  ft <- flextable(data) %>%
    set_header_labels(
      N_numeric = "N",
      Population = "T11",  # This is just the display label, but column remains "Population"
      Total_Rows = "N Reps",
      Total_Violations = "V",
  ...
  ft <- width(ft, j = "Population", width = 0.6)  # ✅ Use "Population" instead of "T11"
...
# Modify background color for 3-Timepoint Models
...
save_as_image(violation_summary_table, path = here('Simulations', 'STUDY_1', "3 Time Points", "zErrors", "3t_l_R_e&v_1.svg")
   )
```
...
save_as_image(violation_summary_table, path = here('Simulations', 'STUDY_1', "3 Time Points", "zErrors", "z3t_l_R_e&v_FINAL.svg")
   )
```
