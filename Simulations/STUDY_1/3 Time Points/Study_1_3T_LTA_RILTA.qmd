...
subtitle: "Study 1.6: LTA Generated, RILTA Analyzed with Three Timepoints"
...
# Main Computer (N = 2000, TPs = 1.385, 0.85, 0.41, -0.41, -0.85, -1.385; N = 4000, TP = 1.385)
#p1 <- p1[(p1$N == 2000 & p1$TPs %in% c(1.385, 0.85, 0.41, -0.41, -0.85, -1.385)) | (p1$N == 4000 & p1$TPs == 1.385), ]

# Old Rig 1 (N = 4000, TP = -1.385)
#p1 <- p1[p1$N == 4000 & p1$TPs == -1.385, ]

# Old Rig 2 (N = 4000, TP = 0.85)
#p1 <- p1[p1$N == 4000 & p1$TPs == 0.85, ]

# Old Rig 3 (N = 4000, TP = -0.85)
#p1 <- p1[p1$N == 4000 & p1$TPs == -0.85, ]

# Old Rig 4 (N = 4000, TP = 0.41)
#p1 <- p1[p1$N == 4000 & p1$TPs == 0.41, ]

# Old Rig 5 (N = 4000, TP = -0.41)
#p1 <- p1[p1$N == 4000 & p1$TPs == -0.41, ]
```
...
lta_rilta_func <- function(N, TPs) {
  ...
  LTA_RILTA <- mplusObject(
    TITLE = glue("Generate LTA_RILTA_N_{N}_TP_{TPs}_1"),
...
# Write CSV
write.csv(violation_summary, here("Simulations", "STUDY_1", "3 Time Points", 'zdata', "violation_summary_l_r_3t.csv"), row.names = FALSE)
```

```{r}

# READ CSV
violation_summary <- read.csv(here("Simulations", "STUDY_1", "3 Time Points", 'zdata', "violation_summary_l_r_3t.csv"))
...
# Main Computer: 6 conditions (N_numeric = 2500, TPs = 1.385, 0.85, 0.41, -0.41, -0.85, -1.385) - 23 cores, PROCESSES = 23
# violation_summary <- violation_summary[(violation_summary$N_numeric == 2000 & violation_summary$TPs %in% c(1.385, 0.85, 0.41, -0.41, -0.85, -1.385)), ]

# Old Rig 1: 2 conditions (N_numeric = 1000, TPs = 1.385, 0.85) - 5 cores, PROCESSES = 5
# violation_summary <- violation_summary[(violation_summary$N_numeric == 1000 & violation_summary$TPs %in% c(1.385, 0.85)), ]

# Old Rig 2: 2 conditions (N_numeric = 1000, TPs = 0.41, -0.41) - 5 cores, PROCESSES = 5
# violation_summary <- violation_summary[(violation_summary$N_numeric == 1000 & violation_summary$TPs %in% c(0.41, -0.41)), ]

# Old Rig 3: 2 conditions (N_numeric = 1000, TPs = -0.85, -1.385) - 5 cores, PROCESSES = 5
# violation_summary <- violation_summary[(violation_summary$N_numeric == 1000 & violation_summary$TPs %in% c(-0.85, -1.385)), ]

# Old Rig 4: 3 conditions (N_numeric = 500, TPs = 1.385, 0.85, 0.41) - 5 cores, PROCESSES = 5
# violation_summary <- violation_summary[(violation_summary$N_numeric == 500 & violation_summary$TPs %in% c(1.385, 0.85, 0.41)), ]

# Old Rig 5: 3 conditions (N_numeric = 500, TPs = -0.41, -0.85, -1.385) - 5 cores, PROCESSES = 5
...
invisible(save_as_image(violation_summary_table, path = here('Simulations', 'STUDY_1', "3 Time Points", "zErrors", "3t_l_R_e&v_1.svg")
   ))
```
...
lta_rilta_func <- function(N_numeric, TPs, Adjusted_Replications_Needed) {
  ...
  # Run Mplus model
  LTA_RILTA_Model<- mplusModeler(LTA_RILTA, 
                                   dataout = here("Simulations", "STUDY_1", "3 Time Points", "6_3T_LTA_GEN_RILTA_ANALYZED_REP", glue("LTA_RILTA_N_{N_numeric}_TP_{TPs}_TH_1.dat")),
                                   modelout = glue(here("Simulations", "STUDY_1", "3 Time Points", "6_3T_LTA_GEN_RILTA_ANALYZED_REP", "LTA_RILTA_N_{N_numeric}_TP_{TPs}_TH_1.inp")),
                                   check = TRUE, run = TRUE, hashfilename = FALSE)
return(LTA_RILTA_Model)
}
...
# Step 1: Set the correct CSV directory
csv_directory <- here('Simulations', 'STUDY_1', '3 Time Points',  '6_3T_LTA_GEN_RILTA_ANALYZED_REP2')
...
# Step 1: Set the correct output directory for .out files
output_folder <- here('Simulations', 'STUDY_1', '3 Time Points', '6_3T_LTA_GEN_RILTA_ANALYZED_REP2')
...
write.csv(violation_summary2, here("Simulations", "STUDY_1", "3 Time Points", "zdata", "violation_summary2_l_r_3t.csv"), row.names = FALSE)
```
...
violation_summary2 <- read.csv(here("Simulations", "STUDY_1", "3 Time Points", "zdata", "violation_summary2_l_r_3t.csv"))
...
save_as_image(violation_summary_table2, path = here('Simulations', 'STUDY_1', "3 Time Points", "zErrors", "z3t_l_R_e&v_2L.svg")
   )
```
...
write.csv(violation_summary_final, here("Simulations", "STUDY_1", "3 Time Points", "zdata", "violation_summary_final_l_r_3t.csv"), row.names = FALSE)
...
violation_summary_final <- read.csv(here("Simulations", "STUDY_1", "3 Time Points", "zdata", "violation_summary_final_l_r_3t.csv"))
...
violation_summary_final <- violation_summary_final %>%
 ...
 create_flextable <- function(data) {
  ...
  ft <- flextable(data) %>%
    ...
    compose(part = "header", j = "LL_Failure_Rate", value = as_paragraph(as_i("ℒ"), as_sub("r"), "%")) %>%
    ...
    compose(part = "header", j = "Final_LL_Failure_Rate", value = as_paragraph(as_i("ℒ"), as_sub("r"), "%")) %>%
...
ft <- ft %>%
  ...
  # ✅ Reapply group-specific bottom borders manually
  ...
  # ✅ Add top + bottom of entire table (APA lines)
  hline_top(border = thin, part = "all") %>%
  hline_bottom(border = thin, part = "all") %>%

  # ✅ Clean font, layout, and alignment
...
invisible(save_as_image(violation_summary_final_table, path = here('Simulations', 'STUDY_1', "3 Time Points", "zErrors", "zl_r_e&v_FINAL.svg")
   ))
```
...
violation_summary_final <- read.csv(
  here("Simulations", "STUDY_1", "3 Time Points", "zdata", "violation_summary_final_l_r_3t.csv")
) %>%
...
invisible(ggsave(
  filename = here("Simulations", "STUDY_1", "3 Time Points", "zposthoc", "l_r_3t_violation_plot.svg"),
  plot = violation_plot,
  width = 7,
  height = 5,
  units = "in",
...
invisible(save_as_image(
  ...
  path = here("Simulations", "STUDY_1", "3 Time Points", "zposthoc", "l_r_3t_pairwise.svg")
))
...
# Save results
write.csv(final_mc_values, here("Simulations", "STUDY_1", "3 Time Points", "zbootstrapping", "l_r_3t_mc_final_mc_values.csv"), row.names = FALSE)
write.csv(boot_results, here("Simulations", "STUDY_1", "3 Time Points", "zbootstrapping", "l_r_3t_boot_results.csv"), row.names = FALSE)
...
final_mc_values <- read.csv(here("Simulations", "STUDY_1", "3 Time Points", "zbootstrapping", "l_r_3t_mc_final_mc_values.csv")) %>%
...
# Debug: Verify N exists
str(final_mc_values$N) # Should show Factor w/ 4 levels "500","1000","2000","4000"
...
# Debug: Check emmeans output columns
print(colnames(as.data.frame(pairwise_bias_N_results))) # Should include lower.CL, upper.CL
...
```{r}
#| label: "save-anova-bias-figure"
#| echo: true
#| message: false
#| warning: false

invisible(ggsave(
  filename = here("Simulations", "STUDY_1", "3 Time Points", "zposthoc", 'l_r_3t_bias_visuals', "l_r_3t_ANOVA_BIAS_plot.svg"),
  plot = bias_plot,
  width = 7,
  height = 5,
  units = "in",
...
**Create Pairwise Comparisons Table for Population**
...
pairwise_df <- pairwise_df %>%
  dplyr::mutate(
    contrast_formatted = paste0("T₁₁ = .", left_right[, 2], " - T₁₁ = .", left_right[, 3])
  )
...
ft <- flextable(pairwise_clean) %>%
  ...
  # Italicize T in Contrast column
  compose(
    j = "Contrast",
    value = as_paragraph(
      as_i("T"), "₁₁ = .", left_right[, 2], " – ",
...
Create pairwise comparison table for N
...
Save population pairwise comparison table
...
invisible(save_as_image(
  ft_bias,
  path = here("Simulations", "STUDY_1", "3 Time Points", "zposthoc", 'l_r_3t_bias_visuals', "l_r_3t_pairwise_bias_population.svg")
))

# Save N pairwise comparisons table
invisible(save_as_image(
  ft_N,
  path = here("Simulations", "STUDY_1", "3 Time Points", "zposthoc", 'l_r_3t_bias_visuals', "l_r_3t_pairwise_bias_N.svg")
))
```
...
ggsave(here('Simulations', 'STUDY_1', '3 Time Points', "zFigures", "x3t_lta_rilta_plots", "plot_mover.svg"), plot = plot_mover_no_title, width = 6, height = 3, dpi = 300, device = "svg")
```
...
ggsave(here('Simulations', 'STUDY_1', '3 Time Points', "zFigures", "x3t_lta_rilta_plots", "plot_stayer.svg"), plot = plot_stayer_no_title, width = 6, height = 3, dpi = 300, device = "svg")
```
...
# Step 2: Assign N_Label only once for each N within Movers and Stayers, and format with markdown italic for N
movers <- movers %>%
  group_by(N) %>%
  mutate(N_Label = ifelse(row_number() == 2, paste0("*N* = ", gsub("N = ", "", N)), "")) %>%
  ungroup()
...
create_gt_table <- function(subset, transition_probability) {
  ...
  gt_table <- subset %>%
    ...
    fmt_markdown(columns = "N_Label") %>%   ### ← THIS LINE RIGHT HERE
    tab_spanner(
      label = "Bias",
