...
summary(mc_values\$Parameter_Bias)

summary(mc_values\$SE_Bias)
...
```{r}
negative_bias <- mc_values %>%
  filter(Parameter_Bias < 0)
print(negative_bias)


negative_bias %>%
  group_by(N) %>%
  summarize(count = n(), mean_bias = mean(Parameter_Bias))


negative_bias %>%
  select(True_Parameter = Population, Estimated_Parameter = average)
