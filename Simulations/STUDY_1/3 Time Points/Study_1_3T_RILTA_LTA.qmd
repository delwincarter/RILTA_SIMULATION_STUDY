...
rilta_lta_func <- function(N, TPs, Lambda) {
  ...
  RILTA_LTA <- mplusObject(
    TITLE = glue("Generate RILTA_LTA_N_{N}_TP_{TPs}_TH_1"),
  ...
  RILTA_LTA_Model<- mplusModeler(RILTA_LTA, 
                                   dataout = here('Simulations', 'STUDY_1', '3 Time Points', "8. 3T RILTA GEN LTA ANALYZED", glue("RILTA_LTA_N_{N}_TP_{TPs}_TH_1_Lambda_{Lambda}.dat")),
                                   modelout = here('Simulations', 'STUDY_1', '3 Time Points', "8. 3T RILTA GEN LTA ANALYZED", "RILTA_LTA_N_{N}_TP_{TPs}_TH_1_Lambda_{Lambda}.inp")),
...
output_folder <- here('Simulations', 'STUDY_1', '3 Time Points', '7_3T_RILTA_GEN_LTA_ANALYZED')  # Adjust to your folder path
...
rilta_lta_func <- function(N_numeric, TPs, Lambda, Replications_Needed) {
  ...
  # Run Mplus model
  RILTA_LTA_Model<- mplusModeler(RILTA_LTA, 
                                   dataout = here('Simulations', 'STUDY_1', '3 Time Points', '8_3T_RILTA_GEN_RILTA_ANALYZED_REP', glue("RILTA_LTA_N_{N_numeric}_TP_{TPs}_TH_1_Lambda_{Lambda}.dat")),
                                   modelout = here('Simulations', 'STUDY_1', '3 Time Points', '8_3T_RILTA_GEN_RILTA_ANALYZED_REP', "RILTA_LTA_N_{N_numeric}_TP_{TPs}_TH_1_Lambda_{Lambda}.inp")),
                                   check = TRUE, run = TRUE, hashfilename = FALSE)
return(RILTA_LTA_Model)
}
...
# Step 4: Parallelized Processing (Windows/Mac/Linux Compatible)
output_folder <- here('Simulations', 'STUDY_1', '3 Time Points', '7_3T_RILTA_GEN_LTA_ANALYZED_REP')  # Adjust to your folder path
file_list <- list.files(output_folder, pattern = "\\.out$", full.names = TRUE)
...
create_plot <- function(data, title_suffix) {
  ...
  base_plot <- ggplot(data = data, aes(x = Lambda, y = Parameter_Bias, color = "Parameter Bias", group = Population_Label)) +  
    ...
    geom_point(aes(y = SE_Bias), shape = 18, size = 1, fill = "#C830CC", alpha = 0.8) +  
