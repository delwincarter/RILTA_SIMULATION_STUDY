...
![](images/clipboard-2674798363-01.png){width="393"}
...
##### Conditions:Sample Size: N = 500, 1000, 2500, 5000
...
# Step 4: Parallelized Processing (Windows/Mac/Linux Compatible)
output_folder <- here('Simulations', 'STUDY_2', "1_LTA_GEN_LTA_ANALYZED_REP")  # Adjust to your folder path
file_list <- list.files(output_folder, pattern = "\\.out$", full.names = TRUE)
...
violation_summary <- violation_summary %>%
  mutate(
    TPs = case_when(
      Population == ".800" ~ formatC(3.179, format = "f", digits = 3),
      Population == ".200" ~ formatC(0.407, format = "f", digits = 3),
      TRUE ~ NA_character_
  ),
