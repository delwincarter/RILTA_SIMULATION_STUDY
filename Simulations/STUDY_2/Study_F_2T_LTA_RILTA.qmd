...
subtitle: "Study 2.2: LTA Generated, RILTA Analyzed with 3 Classes and  2 Timepoints"
...
## Study 2.2: LTA Generated, RILTA Analyzed
...
Transition logit (Probability): TPs = .800 (Stayer), .200 (Mover)
...
# Laptop: 4 conditions (N = 5000, TPs = 3.179 mix 1,2,3 + TPs = 0.407 mix 1) - 11 cores available
p1 <- p1[(p1$N == 5000 & p1$TPs == 3.179 & p1$mix %in% c(1, 2, 3)) | (p1$N == 5000 & p1$TPs == 0.407 & p1$mix == 1), ]  # Uncomment for Laptop

# Desktop 1: 4 conditions (N = 500, TPs = 3.179 mix 1,2,3 + N = 5000, TPs = 0.407 mix 2) - 5 cores available
#p1 <- p1[(p1$N == 500 & p1$TPs == 3.179 & p1$mix %in% c(1, 2, 3)) | (p1$N == 5000 & p1$TPs == 0.407 & p1$mix == 2), ]  # Uncomment for Desktop 1

# Desktop 2: 4 conditions (N = 500, TPs = 0.407 mix 1,2,3 + N = 5000, TPs = 0.407 mix 3) - 5 cores available
#p1 <- p1[(p1$N == 500 & p1$TPs == 0.407 & p1$mix %in% c(1, 2, 3)) | (p1$N == 5000 & p1$TPs == 0.407 & p1$mix == 3), ]  # Uncomment for Desktop 2

# Desktop 3: 4 conditions (N = 1000, TPs = 3.179 mix 1,2,3 + N = 2500, TPs = 0.407 mix 1) - 5 cores available
#p1 <- p1[(p1$N == 1000 & p1$TPs == 3.179 & p1$mix %in% c(1, 2, 3)) | (p1$N == 2500 & p1$TPs == 0.407 & p1$mix == 1), ]  # Uncomment for Desktop 3

# Desktop 4: 4 conditions (N = 1000, TPs = 0.407 mix 1,2,3 + N = 2500, TPs = 0.407 mix 2) - 5 cores available
#p1 <- p1[(p1$N == 1000 & p1$TPs == 0.407 & p1$mix %in% c(1, 2, 3)) | (p1$N == 2500 & p1$TPs == 0.407 & p1$mix == 2), ]  # Uncomment for Desktop 4

# Desktop 5: 4 conditions (N = 2500, TPs = 3.179 mix 1,2,3 + TPs = 0.407 mix 3) - 5 cores available
#p1 <- p1[(p1$N == 2500 & p1$TPs == 3.179 & p1$mix %in% c(1, 2, 3)) | (p1$N == 2500 & p1$TPs == 0.407 & p1$mix == 3), ]  # Uncomment for Desktop 5
...
lta_rilta_func <- function(N, TPs, mix) {
  ...
  LTA_RILTA <- mplusObject(
    ...
    MONTECARLO = glue("NAMES = u11-u15 u21-u25;
      ...
      NREPS = 500;
      !SAVE = repM1*.dat;
      RESULTS = LTA_RILTA_M1_N_{N}_TP_{TPs}_M_{mix}.csv;"),
  ...
  LTA_RILTA_Model <- mplusModeler(
    LTA_RILTA, 
    dataout = here("Simulations", "STUDY_2", "2_LTA_GEN_RILTA_ANALYZED", glue("LTA_RILTA_M1_N_{N}_TP_{TPs}_M_{mix}.dat")),
    modelout = glue(here("Simulations", "STUDY_2", "2_LTA_GEN_RILTA_ANALYZED", "LTA_RILTA_M1_N_{N}_TP_{TPs}_M_{mix}.inp")),
...
invisible(clusterExport(cl, c("lta_rilta_func", "p1")))
...
result_list <- parLapply(cl, 1:nrow(p1), function(i) {
  lta_rilta_func(p1$N[i], p1$TPs[i], p1$mix[i])
})
...
lta_rilta_func2 <- function(N, TPs, mix) {
  ...
  LTA_RILTA2 <- mplusObject(
    TITLE = glue("Generate LTA_RILTA_M2_N_{N}_TP_{TPs}_M_{mix}"),
    ...
    MONTECARLO =
      ...
      NREPS = 500;
      !SAVE = repM2*.dat;
      RESULTS = LTA_RILTA_M2_N_{N}_TP_{TPs}_M_{mix}.csv;"),
  ...
  LTA_RILTA_Model2<- mplusModeler(LTA_RILTA2, 
                                   dataout = here("Simulations", "STUDY_2", "2_LTA_GEN_RILTA_ANALYZED", glue("LTA_RILTA_M2_N_{N}_TP_{TPs}_M_{mix}.dat")),
                                   modelout = glue(here("Simulations", "STUDY_2", "2_LTA_GEN_RILTA_ANALYZED", "LTA_RILTA_M2_N_{N}_TP_{TPs}_M_{mix}.inp")),
                                   check = TRUE, run = TRUE, hashfilename = FALSE)
return(LTA_RILTA_Model2)
}

# Step 3: Export necessary objects to the cluster
invisible(clusterExport(cl, c("lta_rilta_func2", "p1")))
...
result_list <- parLapply(cl, 1:nrow(p1), function(i) {
  lta_rilta_func2(p1$N[i], p1$TPs[i], p1$mix[i])
})
...
lta_rilta_func3 <- function(N, TPs, mix) {
  ...
  LTA_RILTA3 <- mplusObject(
    TITLE = glue("Generate LTA_RILTA_M3_N_{N}_TP_{TPs}_M_{mix}"),
    ...
    MONTECARLO =
      ...
      NREPS = 500;
      !SAVE = repM3*.dat;
      RESULTS = LTA_RILTA_M3_N_{N}_TP_{TPs}_M_{mix}.csv;"),
  ...
  LTA_RILTA_Model3<- mplusModeler(LTA_RILTA3, 
                                   dataout = here("Simulations", "STUDY_2", "2_LTA_GEN_RILTA_ANALYZED", glue("LTA_RILTA_M3_N_{N}_TP_{TPs}_M_{mix}.dat")),
                                   modelout = glue(here("Simulations", "STUDY_2", "2_LTA_GEN_RILTA_ANALYZED", "LTA_RILTA_M3_N_{N}_TP_{TPs}_M_{mix}.inp")),
                                   check = TRUE, run = TRUE, hashfilename = FALSE)
return(LTA_RILTA_Model3)
}

# Step 3: Export necessary objects to the cluster
invisible(clusterExport(cl, c("lta_rilta_func3", "p1")))
...
result_list <- parLapply(cl, 1:nrow(p1), function(i) {
  lta_rilta_func3(p1$N[i], p1$TPs[i], p1$mix[i])
})
...
csv_directory <- here('Simulations', 'STUDY_2', '2_LTA_GEN_RILTA_ANALYZED')
...
output_folder <- here('Simulations', 'STUDY_2', '2_LTA_GEN_RILTA_ANALYZED')
...
violation_summary <- final_data_with_actuals %>%
  ...
  mutate(
    N = as.character(N),
    N_numeric = as.numeric(gsub("N = ", "", N))  # Convert "N = 500" → 500
  ) %>%
...
create_combined_model_data <- function(data, model) {
  data %>%
    ...
    select(
      N_numeric,  
      Population,  # ✅ Changed from `Transition Probability` to `Population`
      Mix,                     
      Total_Violations,  
...
create_flextable <- function(data, title) {
  ...
  ft <- flextable(data) %>%
    set_header_labels(
      N_numeric = "N",     
      Population = "T11",  # ✅ Display label as "T11", but column name remains "Population"
      Mix = "Mix",
      Total_Violations = "V",
      ...
      Adjusted_Replications_Needed = "\u2206 Reps Req’d"
    )
...
#| warning: false
  ...
  # Define the Mplus object with the dynamic replications
lta_rilta_func2 <- function(N_numeric, TPs, Mix, Adjusted_Replications_Needed) {
...
stopCluster(cl)# Stop the cluster after the simulation
```
...
# Step 1: Set the correct CSV directory
csv_directory <- here('Simulations', 'STUDY_2', '2_LTA_GEN_RILTA_ANALYZED_REP')

# Step 2: Source the child document
source(here('Child_Docs', 'data_scraping.R'))
```

*Extract data from the appropriate rows from each 11-row chunk and prepare the data for further processing.*
...
```{r}
#| label: "convert-logits-and-flag-violators2"
#| echo: true
#| message: false
#| warning: false
#| code-fold: true
...
# Step 1: Set the correct output directory for .out files
output_folder <- here('Simulations', 'STUDY_2', '2_LTA_GEN_RILTA_ANALYZED_REP')
...
violation_summary <- final_data_with_actuals %>%
  ...
  mutate(
    ...
    ErrorFlag = ifelse(is.na(ErrorFlag), 0, ErrorFlag)  # Ensure no missing values
  ) %>%
  
  # Step 2: Group and summarize key metrics
  group_by(FileName, Model, Population, N, N_numeric, Mixing_proportion) %>%
  summarize(
    Total_Rows = n(),
    Total_Violations = sum(Any_Violation, na.rm = TRUE),
  ...
  # Step 3: Merge with `replication_summary` to get CompletedReplications
  ...
  mutate(
    ...
    ErrorRate = Total_Errors / Total_Rows,  # Calculate error rate
    Additional_Runs = (500 + Total_Violations) * (Percentage_Violations / 100), 
    Replications_Needed = ceiling(CompletedReplications + Total_Violations + Additional_Runs + 20),
    Adjusted_Replications_Needed = ceiling(Replications_Needed / (1 - ErrorRate))
  ) %>%
  
  # Step 4: Compute `TPs` and `Mix`
  mutate(
    TPs = case_when(
      Population == ".800" ~ 3.179,
      Population == ".200" ~ 0.407,
  ...
  # Step 5: **Ensure `N_numeric` Exists Before Arranging**
  arrange(factor(N_numeric, levels = c(500, 1000, 2500, 5000)), as.numeric(Population)) %>%

  # Step 6: Convert `N` and `Population` back to factors for consistency
  mutate(
    N = factor(N, levels = c("N = 500", "N = 1000", "N = 2500", "N = 5000")),
    Population = factor(Population, levels = c(".200", ".800"))
  ) %>%
  ...
  select(
    FileName, Model, Population, TPs, N, N_numeric, Mix, Mixing_proportion,
    Total_Rows, Total_Violations, Total_Errors, ErrorRate,
    Percentage_Violations, GoodReplications, Replications_Needed, Adjusted_Replications_Needed,
    Reanalysis_Needed
  ) %>%
  
  # Step 8: Ensure `N_numeric` is properly formatted
  arrange(N_numeric, Population) %>%
  mutate(N_numeric = trimws(as.numeric(N_numeric)))   # Ensure it's numeric

# Step 9: Split into separate datasets for each model
model1_data <- violation_summary %>% filter(Model == "Model 1")
model2_data <- violation_summary %>% filter(Model == "Model 2")
model3_data <- violation_summary %>% filter(Model == "Model 3")
...
#| label: "summarize-final-errors-vialtors"
#| echo: true
#| message: false
#| warning: false
#| code-fold: true
...
create_combined_model_data <- function(data, model) {
  data %>%
    ...
    select(
      ...
      Replications_Needed,       
      Adjusted_Replications_Needed,
      GoodReplications   # ✅ Added column for good replications
    )
}
...
create_flextable <- function(data, title) {
  ...
  ft <- flextable(data) %>%
    set_header_labels(
      ...
      Replications_Needed = "Reps Req’d",
      Adjusted_Replications_Needed = "\u2206 Reps Req’d",
      GoodReplications = "Good Reps"  # ✅ Added header for GoodReplications
    )
  ...
  ft <- width(ft, j = "Replications_Needed", width = 0.8)
  ft <- width(ft, j = "Adjusted_Replications_Needed", width = 0.8)
  ft <- width(ft, j = "GoodReplications", width = 0.8)  # ✅ Added width for GoodReplications
...
subset_02_table_even_proportions |> tab_options(table.width = pct(55)) |> gtsave(here('Simulations', 'STUDY_2', 'zHeatmaps', 'z2t_lta_rilta_tables', "2T_L_R_.200, Even Proportions.png"), expand = 8)
```
...
subset_0.2_table_uneven_proportions |> tab_options(table.width = pct(55)) |> gtsave(here('Simulations', 'STUDY_2', 'zHeatmaps', 'z2t_lta_rilta_tables',"2T_L_R_.200, Uneven Proportions.png"), expand = 8)
```
...
subset_0.8_table_even_proportions |> tab_options(table.width = pct(55)) |> gtsave(here('Simulations', 'STUDY_2', 'zHeatmaps', 'z2t_lta_rilta_tables', "2T_L_R_.800, Even Proportions.png"), expand = 8)
```
...
subset_0.8_table_uneven_proportions |> tab_options(table.width = pct(55)) |> gtsave(here('Simulations', 'STUDY_2', 'zHeatmaps', 'z2t_lta_rilta_tables', "2T_L_R_.800, Uneven Proportions.png"), expand = 8)
```
