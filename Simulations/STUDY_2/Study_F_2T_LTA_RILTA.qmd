---
title: "Study Final (k = 3) LTA Generated, RILTA Analyzed"
format:
  html:
    code-fold: true
editor: visual
...
# LTA Generated, RILTA Analyzed

## Introduction

In this section, we will simulate three models by generating them as Latent Transition Models and analyze them as Random Intercepts Latent Transition Models.
...
Transition logit (Probability): TPs = 3.179 (Stayer), .407 (Mover)
...
We will now run a Monte Carlo simulation for Model 1. The simulation will iterate over different combinations of sample sizes, transition probabilities, and mixing proportion components. The following code sets up the cluster for parallel processing and defines the function to run the simulation.
...
lta_rilta_func <- function(N, TPs, mix) {
  ...
  LTA_RILTA_Model <- mplusModeler(
    LTA_RILTA, 
    dataout = here("2_LTA_GEN_RILTA_ANLZE", glue("LTA_RILTA_M1_N_{N}_TP_{TPs}_M_{mix}.dat")),
    modelout = glue(here("2_LTA_GEN_RILTA_ANLZE", "LTA_RILTA_M1_N_{N}_TP_{TPs}_M_{mix}.inp")),
...
# Model 2: LTA Generated, RILTA Analyzed
...
We will now run a Monte Carlo simulation for Model 2. The simulation will iterate over different combinations of sample sizes, transition probabilities, and mixing proportion components. The following code sets up the cluster for parallel processing and defines the function to run the simulation.
...
lta_rilta_func2 <- function(N, TPs, mix) {
  ...
  LTA_RILTA_Model2<- mplusModeler(LTA_RILTA2, 
                                   dataout = here("2_LTA_GEN_RILTA_ANLZE", glue("LTA_RILTA_M2_N_{N}_TP_{TPs}_M_{mix}.dat")),
                                   modelout = glue(here("2_LTA_GEN_RILTA_ANLZE", "LTA_RILTA_M2_N_{N}_TP_{TPs}_M_{mix}.inp")),
                                   check = TRUE, run = TRUE, hashfilename = FALSE)
...
# Model 3: LTA Generated, RILTA Analyzed
...
We will now run a Monte Carlo simulation for Model 3. The simulation will iterate over different combinations of sample sizes, transition probabilities, and mixing proportion components. The following code sets up the cluster for parallel processing and defines the function to run the simulation.
...
lta_rilta_func3 <- function(N, TPs, mix) {
  ...
  LTA_RILTA_Model3<- mplusModeler(LTA_RILTA3, 
                                   dataout = here("2_LTA_GEN_RILTA_ANLZE", glue("LTA_RILTA_M3_N_{N}_TP_{TPs}_M_{mix}.dat")),
                                   modelout = glue(here("2_LTA_GEN_RILTA_ANLZE", "LTA_RILTA_M3_N_{N}_TP_{TPs}_M_{mix}.inp")),
                                   check = TRUE, run = TRUE, hashfilename = FALSE)
...
csv_directory <- here('2_LTA_GEN_RILTA_ANALYZED')
...
output_folder <- "2_LTA_GEN_RILTA_ANALYZED"  # Adjust to your folder path
...
#### Step 5: Part 3 Calculate Violation Percentages per Condition to create new iterators
...
Rerun Analysis *with increased DYNAMIC REPLICATIONS to overcome errors and violations*
...
lta_rilta_func <- function(N_numeric, TPs, Mix, Adjusted_Replications_Needed) {
  ...
  LTA_RILTA_Model <- mplusModeler(
    LTA_RILTA, 
    dataout = here("2_LTA_GEN_RILTA_ANLZE_REP", glue("LTA_RILTA_M1_N_{N_numeric}_TP_{TPs}_M_{Mix}.dat")),
    modelout = glue(here("2_LTA_GEN_RILTA_ANLZE_REP", "LTA_RILTA_M1_N_{N_numeric}_TP_{TPs}_M_{Mix}.inp")),
    check = TRUE, run = TRUE, hashfilename = FALSE
  )
  return(LTA_RILTA_Model)
}
...
lta_rilta_func2 <- function(N_numeric, TPs, Mix, Adjusted_Replications_Needed) {
  ...
  # Run Mplus model
  LTA_RILTA_Model2<- mplusModeler(LTA_RILTA2, 
                                   dataout = here("2_LTA_GEN_RILTA_ANLZE_REP", glue("LTA_RILTA_M2_N_{N_numeric}_TP_{TPs}_M_{Mix}.dat")),
                                   modelout = glue(here("2_LTA_GEN_RILTA_ANLZE_REP", "LTA_RILTA_M2_N_{N_numeric}_TP_{TPs}_M_{Mix}.inp")),
                                   check = TRUE, run = TRUE, hashfilename = FALSE)
return(LTA_RILTA_Model2)
}
...
lta_rilta_func3 <- function(N_numeric, TPs, Mix, Adjusted_Replications_Needed) {
  ...
  # Run Mplus model
  LTA_RILTA_Model3<- mplusModeler(LTA_RILTA3, 
                                   dataout = here("2_LTA_GEN_RILTA_ANLZE_REP", glue("LTA_RILTA_M3_N_{N_numeric}_TP_{TPs}_M_{Mix}.dat")),
                                   modelout = glue(here("2_LTA_GEN_RILTA_ANLZE_REP", "LTA_RILTA_M3_N_{N_numeric}_TP_{TPs}_M_{Mix}.inp")),
                                   check = TRUE, run = TRUE, hashfilename = FALSE)
return(LTA_RILTA_Model3)
}
...
#### TEST CHECK FOR ERRORS AND WARNINGS
...
# Step 4: Parallelized Processing (Windows/Mac/Linux Compatible)
output_folder <- "2_LTA_GEN_RILTA_ANALYZED_REP"  # Adjust to your folder path
file_list <- list.files(output_folder, pattern = "\\.out$", full.names = TRUE)
...
# Ensure consistent column formats
error_summary <- error_summary %>%
  mutate(
    FileName = tolower(str_trim(FileName)),  # Standardize FileName
    Replication = as.character(Replication) # Convert Replication to character
  )
...
create_combined_model_data <- function(data, model) {
  data %>%
    ...
    select(
      ...
      `Total Mplus Runs` = Total_Rows,  # Total number of Mplus runs
      `Total Violations` = Total_Violations,  # Show total violations
      `% of Violations` = Percentage_Violations,  # Show percentage of violations
      `ErrorRate`              # Include the error rate
    )
}
...
create_gt_table <- function(data, title) {
  data %>%
    ...
    tab_header(
      title = title,
      subtitle = "Percentage of Cases with Label Switching and Adjusted Replications Needed"
    ) %>%
    cols_align(align = "center", columns = everything()) %>%
    tab_options(data_row.padding = px(4)) %>%
    tab_style(style = cell_text(align = "center"), locations = cells_column_labels(everything())) %>%
...
### **Step 10: Delete Cases with errors and violations**

#### **Objective**

*Filter out cases with any errors and violations, leaving only the clean data.*
