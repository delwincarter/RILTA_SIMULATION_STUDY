...
lta_rilta_func <- function(N, TPs, TH) {
  ...
  LTA_RILTA_Model<- mplusModeler(LTA_RILTA, 
                                   dataout = here("2 2T LTA GEN RILTA ANALYZED", glue("LTA_RILTA_N = {N}_TP = {TPs}_TH = {TH}_WD.dat")),
                                   modelout = glue(here("2 2T LTA GEN RILTA ANALYZED", "LTA_RILTA_N = {N}_TP = {TPs}_TH = {TH}_WD.inp")),
...
```{r}
# Extract models
lta_sim <- readModels(here("2 2T LTA GEN RILTA ANALYZED"), what = "parameters")
...
common_labels <- labs(
  ...
  y = "Bias (%)",
  color = "",
  title = "LTA Generated and RILTA Analyzed with Stayer Transition Probabilities"
)
