...
rilta_lta_func <- function(N, TPs, TH, lamda) {
  
  RILTA_LTA <- mplusObject(
    TITLE = glue("Generate RILTA_LTA_N = {N}_TP = {TPs}_{TH}"),
  ...
  RILTA_LTA_Model<- mplusModeler(RILTA_LTA, 
                                   dataout = here("8. 3T RILTA GEN LTA ANALYZED", glue("RILTA_LTA_N = {N}_TP = {TPs}_TH = {TH}_lamda = {lamda}.dat")),
                                   modelout = glue(here("8. 3T RILTA GEN LTA ANALYZED", "RILTA_LTA_N = {N}_TP = {TPs}_TH = {TH}_lamda = {lamda}.inp")),
                                   check = TRUE, run = TRUE, hashfilename = FALSE)
return(RILTA_LTA_Model)
}

result_list <- lapply(1:nrow(p1), function(i) {
  rilta_lta_func(p1$N[i], p1$TPs[i], p1$TH[i],p1$lamda[i])
               })
...
```{r}
# Extract models
lta_sim <- readModels(here("8. 3T RILTA GEN LTA ANALYZED"), what = "parameters")
...
plot_mover <- ggplot(data = subset_mover, aes(x = Lambda)) +  
  ...
  scale_color_manual(
    ...
    labels = c("Parameter Bias", "Standard Error Bias", "Coverage Failure"), 
    breaks = c("Parameter Bias", "Standard Error Bias", "Coverage Failure"),
    guide = guide_legend(
      override.aes = list(
...
plot_stayer <- ggplot(data = subset_stayer, aes(x = Lambda)) +  
  ...
  scale_color_manual(
    values = c("Parameter Bias" = "#4169E1", "Standard Error Bias" = "#E14169", "Coverage Failure" = "#4169E1"), 
    labels = c("Parameter Bias", "Standard Error Bias", "Coverage Failure"), 
    breaks = c("Parameter Bias", "Standard Error Bias", "Coverage Failure"),
    guide = guide_legend(
      override.aes = list(
